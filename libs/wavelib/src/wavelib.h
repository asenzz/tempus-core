...
struct wave_set {
    ...
    int hpd_len;    //hp decon len
    int lpr_len;        //lp recon len
    int hpr_len;
    double *lpd;    //lp decon
    double *hpd;
    double *lpr;        //lp recon
    ...
    //F-K filers coeficients
    double fk4params[4*4];
    double fk6params[4*6];   
    double fk8params[4*8];   
    double fk14params[4*14];
    double fk22params[4*22];
};
...
struct wt_set{
    ...
    conv_object cobj;
    char method[10];
    int filterorder;    //use in Fejer - Korovkin wave length fomula
