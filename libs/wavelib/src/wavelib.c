...
void copy_waveset(wave_object src, const wave_object dst)  {
        strncpy(src->wname, dst->wname, 50);
        dst->filtlength = src->filtlength;
        dst->lpd_len = src->lpd_len;
        dst->hpd_len = src->hpd_len;
        dst->lpr_len = src->lpr_len;
        dst->hpr_len = src->hpr_len;
        *dst->lpd = *src->lpd;
        *dst->hpd = *src->hpd;
        *dst->lpr = *src->lpr;
        *dst->hpr = *src->hpr;
...
wave_object wave_init1(int filter) {
   ...
   retval = filtlength1(filter);  
    ...
    if (filter == en_fk4) {
        ...
        filtcoef1(filter, obj->fk4params,  obj->fk4params+retval, obj->fk4params+2*retval, obj->fk4params+3*retval );  

        obj->lpd = &obj->fk4params[0];
        obj->hpd = &obj->fk4params[retval];
        obj->lpr = &(obj->fk4params[2 * retval] );
        obj->hpr = &(obj->fk4params[3 * retval] );
    ...
    if (filter == en_fk6) {
        ...
        filtcoef1(filter, obj->fk6params,  obj->fk6params+retval, obj->fk6params+2*retval, obj->fk6params+3*retval );  

        obj->lpd = &obj->fk6params[0];
        obj->hpd = &obj->fk6params[retval];
        obj->lpr = &(obj->fk6params[2 * retval] );
        obj->hpr = &(obj->fk6params[3 * retval] );
    ...
    if (filter == en_fk8) {
        ...
        filtcoef1(filter, obj->fk8params,  obj->fk8params+retval, obj->fk8params+2*retval, obj->fk8params+3*retval );  

        obj->lpd = &obj->fk8params[0];
        obj->hpd = &obj->fk8params[retval];
        obj->lpr = &(obj->fk8params[2 * retval] );
        obj->hpr = &(obj->fk8params[3 * retval] );
    ...
    if (filter == en_fk14) {
        ...
        filtcoef1(filter, obj->fk14params,  obj->fk14params+retval, obj->fk14params+2*retval, obj->fk14params+3*retval );  

        obj->lpd = &obj->fk14params[0];
        obj->hpd = &obj->fk14params[retval];
        obj->lpr = &(obj->fk14params[2 * retval] );
        obj->hpr = &(obj->fk14params[3 * retval] );
     ...
     if (filter == en_fk22) {
        ...
        filtcoef1(filter, obj->fk22params,  obj->fk22params+retval, obj->fk22params+2*retval, obj->fk22params+3*retval );  

        obj->lpd = &obj->fk22params[0];
        obj->hpd = &obj->fk22params[retval];
        obj->lpr = &(obj->fk22params[2 * retval] );
        obj->hpr = &(obj->fk22params[3 * retval] );
...
wt_object wt_init(wave_object wave, char* method, int siglength, int J) {
	...
	/* TODO: Remove this check as for swt fft conv it is not applicable and in general is not correct...
	 * TODO: The max iterations evaluation could be reworked also...
	 * TODO: For sure remove this nasty exit() and return some error.
	if (J > MaxIter) {
    ...
    */
	...
	if (method == NULL) {
		obj = (wt_object)calloc(sizeof(struct wt_set) + sizeof(double)* (siglength +  2 * J * (size+1)), sizeof(char));
		...
		obj = (wt_object)calloc(sizeof(struct wt_set) + sizeof(double)* (siglength * (J + 1)), sizeof(char));
	...
	if (!strcmp(method, "dwt") || !strcmp(method, "DWT")) {
		...
		for (i = 0; i < siglength * (J + 1); ++i) {
			obj->params[i] = 0.0;
                    ...
                    else if(!strcmp(method, "modwt-fk4")){
...
wtree_object wtree_init(wave_object wave, int siglength,int J) {
	...
	obj = (wtree_object)calloc(sizeof(struct wtree_set) + sizeof(double)* (siglength * (J + 1) + elength + nodes + J + 1), sizeof(char));
	...
	obj->coeflength = (int*)&obj->params[siglength * (J + 1) + elength + nodes];
                     ...
                     //memset(&obj->params[0], 0, siglength * (J + 1) + elength + nodes + J + 1);
...
wpt_object wpt_init(wave_object wave, int siglength, int J) {
	...
	obj = (wpt_object)calloc(sizeof(struct wpt_set) + sizeof(double)* (elength + 4 * nodes + 2 * J + 6), sizeof(char));
...
cwt_object cwt_init(char* wave, double param,int siglength, double dt, int J) {
	...
	N = siglength;
	nj2 = 2 * N * J;
	obj = (cwt_object)calloc(sizeof(struct cwt_set) + sizeof(double)* (nj2 + 2 * J + N), sizeof(char));
...
// modwt_per1 is test fuction for F-K case
static void modwt_per1(wt_object wt, int M, double *inp, int N, double *cA, int len_cA, double *cD, int len_cD) {
    int l, i, t, len_avg;
    double s;
    double *filt;
...
void modwt1(wt_object wt, double *inp) {
    ...
    for (iter = 0; iter < J; ++iter) {
        ...
        modwt_per1(wt, M, wt->params, temp_len, cA, temp_len, cD, temp_len);
...
// imodwt_per1 is test fuction for F-K case
static void inverse_imodwt_per(wt_object wt,int M, double *cA, int len_cA, double *cD, int len_cD, double *X) {
...
// modwt is test fuction for F-K case
void inverse_modwt(wt_object wt, double *dwtop) {
    ...
    for (iter = 0; iter < J; ++iter) {
            ...
            inverse_imodwt_per(wt, M, dwtop, N, wt->params + lenacc, N, X);
            for (j = 0; j < N; ++j) {
                    dwtop[j] = X[j];
            }
...
void wtree_free(wtree_object object) {
    if (object->wave) wave_free(object->wave);
    object->wave = NULL;
    if (object->cobj) free_conv(object->cobj);
    object->cobj = NULL;
	free(object);
}
