			...
			for(int k=0;k<dim;k++){
 				double bs_sum=0.;
	                        for(int m=0;m<len;m++){
                	                ...
                	                bs_sum += (X[m+k*len+total_len_features*i] - X[m+k*len+total_len_features*j])*(X[m+k*len+total_len_features*i] - X[m+k*len+total_len_features*j])*powers[m];
                        	}
	                        double cum_sum1=0.;
        	                double cum_sum2=0.;
...
__global__  void  gpu_kernel_xx_compute (size_t sizeX ,size_t startX, size_t startY, size_t numX, size_t numY, int len, int dim, const double*X, double *Z,size_t full_sizeZ,double param1, double param2, double param3, double param4){
...
__global__  void  gpu_kernel_xy_compute (size_t sizeX ,size_t sizeY,size_t startX, size_t startY,size_t numX, size_t numY,int len, int dim, const double*X,const double*Y, double *Z,size_t full_sizeZ,double param1, double param2, double param3,double param4){
...
int do_gpu_kernel_compute_mat_xx(size_t sizeX , size_t startX,size_t startY,size_t numX,size_t numY, size_t total_len_features, int dim, const double*X, double *Z,double*&d_Zptr, /* double param1, */ double param2, double param3,double param4){
...
int do_gpu_kernel_compute_mat_xy(size_t sizeX , size_t sizeY, size_t startX,size_t startY, size_t numX,size_t numY, size_t total_len_features, int dim, const double*X,const double*Y, double *Z,double*&d_Zptr,double param1, double param2, double param3,double param4){
        size_t len=total_len_features/dim;
        assert(len*dim==total_len_features);
	...
	assert(tile_x==tile_y);
  	...
  	int block_x = (sizeX / tile_x) + (sizeX % tile_x ==0? 0:1);
        ...
        gpu_kernel_xy_compute<<<block_dim,thread_dim>>> ( sizeX,sizeY,startX,startY,numX,numY,len,dim,d_Xptr,d_Yptr,d_Zptr,full_sizeZ,param1,param2,param3,param4);
        gpuErrchk( cudaPeekAtLastError() );
        gpuErrchk( cudaDeviceSynchronize() );
        ctimes[4]+=msecs()-times4;
        double times5=msecs();
...
int  cuda_kernel_xx(size_t total_len_features,int dim,size_t size_X,size_t startX,size_t startY, size_t numX, size_t numY, const double*X,/*double sigma,*/double lambda,double tau,double w_sum_sym,double*Z,double*&d_Z_ptr){
//    double param1=sigma;
...
int  cuda_kernel_xy(size_t total_len_features,int dim,size_t size_X,size_t size_Y,size_t startX,size_t startY, size_t numX, size_t numY, const double*X,const double*Y,double sigma,double lambda,double tau,double w_sum_sym,double*Z,double*&d_Z_ptr){
        double param1=sigma;
        double param2=lambda;
        double param3=tau;
	double param4=w_sum_sym;
