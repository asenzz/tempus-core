...
file_name=dir_name=os.path.basename(orig_dataset_file)
dataset_file="/tmp/"+file_name+".clean.txt"
...
os.system("sed 's/.*:00 //' " + orig_dataset_file + " > " + dataset_file)
os.system("awk 'NR % 9 != 0 && NR % 8 != 0 && NR % 7 != 0' " + dataset_file + " > " + train_rows)
os.system("awk 'NR % 9 == 0 || NR % 8 == 0 || NR % 7 == 0' " + dataset_file + " > " + validate_rows)
kernel_type= "-t " + sys.argv[2]
gamma_arg = "-g " + sys.argv[3]
epsilon_arg = "-p " + sys.argv[4]
cost_arg = "-c " + sys.argv[5]

#print("./svm-train -h 0 -s 3 "+ kernel_type+" " + gamma_arg+" "+epsilon_arg+" "+cost_arg+" "+train_rows+" "+model_file)
