...
{
    LOG_INFO("nearestBarTimeOrBefore", "Starting " + TimeToString(bar_time, TIME_DATE_SECONDS));
    ...
    LOG_INFO("", "Returning " + TimeToString(result, TIME_DATE_SECONDS) + " for bar time " + TimeToString(bar_time, TIME_DATE_SECONDS));
    return result;
} 
...
class SVRApi
{
    ...
    bool get_missing_period(datetime& time_from, datetime& time_to, const string &input_queue, const string &period, const string &function, bool &recon_finished);
...
public:
    ...
    static int copy_ticks_safe(const string &symbol, MqlTick &ticks[], const uint flags, const datetime timeStart, const datetime timeEnd);
    ...
    const string &input_queue, const string &period, const bool finalize, const aprice &op, const aprice &hi, const aprice &lo, const aprice &cl, const uint vol, const datetime tm)
{
    ...
    const string time_str = TimeToString(tm, TIME_DATE_SECONDS);
    ...
    param.hPutString("clientTime", TimeToString(TimeLocal(), TIME_DATE_SECONDS));
...
void SVRApi::send_bar(const string &input_queue, const string &period, const bool finalize, const AveragePrice &avg_price)
{
    ...
    uint bar = 0;
    const string time_str = TimeToString(avg_price.tm, TIME_DATE_SECONDS);
    param.hPutString("symbol", input_queue);
    param.hPutString("time", time_str);
...
void SVRApi::send_bars(const string &input_queue, const string &period, const bool finalize, const aprice &prices[], const datetime &times[], const uint &volumes[])
{
    ...
    const string input_queue_bid_name = input_queue + "_bid";
    const string input_queue_ask_name = input_queue + "_ask";
    Hash param;
    param.hPutString("period", period);
    param.hPutString("clientTime", TimeToString(TimeLocal(), TIME_DATE_SECONDS));
    param.hPutString("symbol", input_queue);
    ...
    for (int i = 0; i < price_len; ++i) {
        param.hPutString("time", TimeToString(times[i], TIME_DATE_SECONDS));
        param.hPutString(input_queue_bid_name, DoubleToString(prices[i].bid, DOUBLE_PRINT_DECIMALS));
        param.hPutString(input_queue_ask_name, DoubleToString(prices[i].ask, DOUBLE_PRINT_DECIMALS));
    ...
    const string &aux_col_name)
{
    ...
    static const string send_tick = "sendTick";
    const string current_time_str = TimeToString(TimeLocal(), TIME_DATE_SECONDS);
...
bool SVRApi::next_time_range(datetime& time_from, datetime& time_to, const string &input_queue, const string &period)
{
    ...
    const bool result = get_missing_period(time_from, time_to, input_queue, period, funName, reconFinished);
    if (result) LOG_INFO("", "The requested interval is: " + TimeToString(time_from, TIME_DATE_SECONDS) + " to " + TimeToString(time_to, TIME_DATE_SECONDS));
    return result;
}
...
//|                                                                  |
//+------------------------------------------------------------------+
int SVRApi::copy_ticks_safe(const string &symbol, MqlTick &ticks[], const uint flags, const datetime timeStart, const datetime timeEnd)
{
    int last_error, copied_ct;
    uint retries = 0;
    do {
        copied_ct = CopyTicksRange(symbol, ticks, flags, timeStart * 1000, timeEnd * 1000);
        if (copied_ct >= 0) return copied_ct;
        last_error = GetLastError();
        ResetLastError();
    } while (copied_ct < 0 && ++retries < C_retries_count &&
            (last_error == ERR_HISTORY_TIMEOUT || last_error == ERR_HISTORY_LOAD_ERRORS || last_error == ERR_HISTORY_NOT_FOUND));
    LOG_ERROR("",
        "Failed to copy ticks for " + symbol + ", from " + TimeToString(timeStart, TIME_DATE_SECONDS) + ", to " +
        TimeToString(timeEnd, TIME_DATE_SECONDS) + ", error " + ErrorDescription(last_error));
...
int SVRApi::copy_rates_safe(const string &symbol, const ENUM_TIMEFRAMES time_frame, const datetime start, const datetime end, MqlRates &rates[])
{
    ...
    int copied_ct = CopyRates(copied_symbol, time_frame, start, end, rates);
    int last_error;
    if (copied_ct < 0) {
        last_error = GetLastError();
        ResetLastError();
        uint retries = 0;
        while (last_error == ERR_HISTORY_LOAD_ERRORS || last_error == ERR_HISTORY_TIMEOUT || last_error == ERR_HISTORY_NOT_FOUND) {
            LOG_ERROR("", "Retrying last_error " + ErrorDescription(last_error) + " for " + copied_symbol);
            Sleep(10);
            copied_ct = CopyRates(copied_symbol, time_frame, start, end, rates);
...
{    
    ...
    if (start_time >= end_time) {
        LOG_ERROR("", "Illegal time range " + TimeToString(start_time, TIME_DATE_SECONDS) + " until " + TimeToString(end_time, TIME_DATE_SECONDS));
        return 0;
    }
    const int start_bar = iBarShift(_Symbol, _Period, start_time, false);
    if (start_bar < 0) {
        LOG_ERROR("", "Bar for start time " + TimeToString(start_time, TIME_DATE_SECONDS) + " not found: " + ErrorDescription(GetLastError()));
        ResetLastError();
        return 0;
    } else if (iTime(_Symbol, _Period, start_bar) != start_time) {
        LOG_ERROR("", "Start time " + TimeToString(start_time, TIME_DATE_SECONDS) + " does not equal bar time " + TimeToString(iTime(_Symbol, _Period, start_bar), TIME_DATE_SECONDS));
    }
    const int end_bar = iBarShift(_Symbol, _Period, end_time, false);
    if (end_bar < 0) {
        LOG_ERROR("", "Bar for end time " + TimeToString(end_time, TIME_DATE_SECONDS) + " not found: " + ErrorDescription(GetLastError()));
        ResetLastError();
        return 0;
    } else if (iTime(_Symbol, _Period, end_bar) != end_time)
        LOG_INFO("", "End time " + TimeToString(end_time, TIME_DATE_SECONDS) + " does not equal bar time " + TimeToString(iTime(_Symbol, _Period, end_bar), TIME_DATE_SECONDS));

    // end_bar is smaller than start_bar
    const uint bar_ct = (per_second ? C_period_seconds : 1) * (1 + start_bar - end_bar);
    LOG_INFO("", "Preparing up to " + string(bar_ct) + " bars for period " + TimeToString(start_time, TIME_DATE_SECONDS) + " to " + TimeToString(end_time, TIME_DATE_SECONDS));
    ...
    for (int bar_ix = start_bar; bar_ix >= end_bar; --bar_ix) {
        ...
        const string bar_time_str = TimeToString(bar_time, TIME_DATE_SECONDS);
        if (bar_time == 0) {
            LOG_ERROR("", "Bar time or price not found for " + _Symbol + ", from " + bar_time_str + ", to " + TimeToString(next_bar_time, TIME_DATE_SECONDS) + 
        ...
        if (ticks_count < 1) {
            if (iHigh(_Symbol, _Period, bar_ix) != iLow(_Symbol, _Period, bar_ix) || iOpen(_Symbol, _Period, bar_ix) != iClose(_Symbol, _Period, bar_ix)) {
                LOG_ERROR("", "Inconsistency between ticks and bars at " + bar_time_str + ", high doesn't equal low but no ticks for bar copied. Using one minute rates for " + bar_time_str + " which is imprecise!");
                MqlRates rates[];
        ...
        if (per_second) {
            ...
            if (ticks_count < 1) {
                LOG_ERROR("", "No ticks found for " + bar_time_str + " using close price for bar " + string(bar_ix));
            ...
            if (!current_price.value.valid()) {
                LOG_ERROR("", "Illegal price for " + TimeToString(bar_time, TIME_DATE_SECONDS));
                continue;
            }
            myAvg[ok_bars] = current_price.value;
            myTm[ok_bars] = current_price.tm;
    ...
    LOG_INFO("", "Successfully prepared " + IntegerToString(ok_bars) + " bars out of " + IntegerToString(bar_ct) + " possible bars from " + 
                TimeToString(start_time, TIME_DATE_SECONDS) + " until " + TimeToString(end_time, TIME_DATE_SECONDS));
    return ok_bars;
}
...
int SVRApi::prepare_history_ohlc(const datetime start_time, const datetime end_time)
{
    ...
    LOG_INFO("", "Preparing from " + TimeToString(start_time, TIME_DATE_SECONDS) + " until " + TimeToString(end_time, TIME_DATE_SECONDS));
    const int start_bar = iBarShift(_Symbol, _Period, start_time, false);
    if (start_bar < 0) {
    ...
    LOG_INFO("", "Copied " + string(bar_ct) + " bars from " + TimeToString(start_time, TIME_DATE_SECONDS) + " until " + TimeToString(end_time, TIME_DATE_SECONDS));
    return bar_ct;
}
...
bool SVRApi::send_history(const string &input_queue, const string &period, const datetime start_time, datetime end_time)
{
    ...
    const datetime time_current = TimeLocal();
    const string time_current_str = TimeToString(time_current, TIME_DATE_SECONDS);
    if (file_handle != INVALID_HANDLE) {
        FileSeek(file_handle, 0, SEEK_END);
        if (is_average)
            for (int bar_ix = 0; bar_ix < bar_count; ++bar_ix)
                FileWrite(file_handle, TimeToString(myTm[bar_ix], TIME_DATE_SECONDS), time_current_str, IntegerToString(myVl[bar_ix]), 
...
uint SVRApi::pack_history_ohlc(const uint bar_start, const uint bar_end, const string &delimiter, Hash &hash)
{
    ...
    for (uint pack_bar = 0; bar_ix <= bar_end && pack_bar < packSize; ++bar_ix, ++pack_bar)
        hash.hPutString(IntegerToString(bar_ix), 
            TimeToString(myTm[bar_ix], TIME_DATE_SECONDS) + time_zone_delim + 
...
uint SVRApi::pack_history_avg(const uint bar_start, const uint bar_end, const string &delimiter, Hash &hash)
{
    ...
    if (AuxInputQueue != "") {
        for (uint pack_bar = 0; bar_ix <= bar_end && pack_bar < packSize; ++bar_ix, ++pack_bar)
            hash.hPutString(
                IntegerToString(bar_ix), TimeToString(myTm[bar_ix], TIME_DATE_SECONDS) + delimiter + 
...
//+------------------------------------------------------------------+
bool SVRApi::get_missing_period(datetime& time_from, datetime& time_to, const string &input_queue, const string &period, const string &function, bool &recon_finished)
{
    string response;
    Hash params;
    params.hPutString("symbol", input_queue);
    params.hPutString("period", per_second ? "1" : period);
    const int bars = BARS_OFFERED < iBars(_Symbol, _Period) ? BARS_OFFERED : iBars(_Symbol, _Period);
    time_to = iTime(_Symbol, _Period, 1);
    if (per_second) time_to += C_period_seconds - 1;
    time_from = iTime(_Symbol, _Period, bars - 1);
#ifdef HISTORYTOSQL
    return true;
#endif
    const string strOfferToTime = TimeToString(time_to, TIME_DATE_SECONDS);
    const string strOfferFromTime = TimeToString(time_from, TIME_DATE_SECONDS);
    params.hPutString("offeredTimeFrom", strOfferFromTime);
    params.hPutString("offeredTimeTo", strOfferToTime);
    LOG_INFO("", "Offering " + string(per_second ? bars * C_period_seconds : bars) + " bars for " + input_queue + ", symbol " + _Symbol + 
                    ", from " + strOfferFromTime + " until " + strOfferToTime);
    const string queue_call = "queue";
    ...
    if (!success)
        ...
        if (!jo.getString("time_from", strReqTimeFrom)) recon_finished = true;
        jo.getString("time_to", strReqTimeTo);
