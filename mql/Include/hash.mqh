...
class HashDouble : public HashValue {
    ...
    string toString(){ return(DoubleToString(getVal())); }
};
...
class HashInt : public HashValue {
    ...
    string toString() { return(IntegerToString(getVal())); }
};
...
class HashDatetime : public HashValue {
    ...
    string toString() { return (TimeToString(getVal())); }
};
...
public:
    ...
    HashEntry*getEntry(const int i) const {
        return _buckets[i];
    }
    ...
    bool rehash(uint newSize) {
        ...
        if (newSize <= getSlots()) {
            ...
            for(uint ij = 0 ; ij<newSize ; ij++ ) _buckets[ij] = NULL;
