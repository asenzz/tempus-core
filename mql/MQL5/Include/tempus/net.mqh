...
bool ParseUrl(string Url, ParsedUrl &purl)
{
    ...
    const int k = StringSplit(Url, ':', result);

    if (k == 0) {
        LOG_ERROR("An empty Url passed");
        return false;
    }

    if(k < 0) {
        LOG_ERROR(ErrorDescription(GetLastError()));
        return false;
    }

    if(k > 3) {
        LOG_ERROR("only two columns are allowed in the URL" + Url);
        return false;
    }
    ...
    if( k > hostIndex + 1 ) port = StringToInteger(result[hostIndex + 1]);
...
//|                                                                  |
//+------------------------------------------------------------------+
const string C_headers_json = "Content-Type: application/json\r\n";
const int C_len_headers_json = StringLen(C_headers_json);
const string C_post_verb = "POST";
string C_http_ver = "HTTP/1.1";
string C_empty_string = "";
const int C_web_request_timeout = 120;
string C_cookie_header = "cppcms_session=";
string head_www_form = "Content-Type: application/x-www-form-urlencoded";
const int len_head = StringLen(head_www_form);
string C_accept_types[] = { "Accept: text/*\r\n" };
string C_user_agents = "Mozilla";
...
bool MqlNet::Open(const string &host, const int port, const bool secure)
{
    ...
    Session = (int) InternetOpenW(C_user_agents, OPEN_TYPE_PRECONFIG, C_empty_string, C_empty_string, 0); // open session
...
// send request
    ...
    res = StringFind(responseHeaders, C_cookie_header);

    if (res >= 0) { // SetCookie header received
        auth = StringSubstr(responseHeaders, res + StringLen(C_cookie_header));
        LOG_DEBUG("SetCookie auth header received: " + auth);
        session_cookie = C_cookie_header + StringSubstr(auth, 0, StringFind(auth, ";"));
...
// hRequest = HttpOpenRequestW(Connect, C_post_verb, this_path, C_http_ver, C_empty_string, 0,  RegularConnectionFlags | ( INTERNET_FLAG_SECURE * session_secure ), 0);
    ...
    while (++retries < 10 && !hRequest)
        hRequest = HttpOpenRequestW(Connect, C_post_verb, this_path, C_http_ver, 0, 0, flags, 0);
    if (!hRequest) {
        LOG_SYS_ERR("HttpOpenRequestW failed");
        Close();
        return false;
