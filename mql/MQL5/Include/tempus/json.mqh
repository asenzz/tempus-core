...
public:
    ...
    ENUM_JSON_TYPE getType() {
        return _type;
    }

    void setType(ENUM_JSON_TYPE t) {
        _type = t;
    }
    ...
    string toString() {
        ...
        if (_hash != NULL) {
            ...
            for (l = new HashLoop(_hash); l.hasNext(); l.next()) {
                ...
                StringAdd(s, (++n == 1 ? "" : ",") + "\"" + l.key() + "\" : " + v.toString());
            }
            delete l;
        }
        s = s + "}";
...
public:
    ...
    string toString() {
        string s = "[";
        if (_size > 0) {
            StringAdd(s, _array[0].toString());
            for (int i = 1; i < _size; i++) {
                string substr = "," + _array[i].toString();
                StringAdd(s, substr);
            }
        }
        s = s + "]";
...
public:
    ...
    JSONValue *parse(string s) {
        ...
        if (_len != inLen + 1 /* nul */ ) {
            string substr = "unable to create array " + string(inLen) + " got " + string(_len);
            setError(1, substr);
        } else {
            _len--;
            ret = parseValue();
