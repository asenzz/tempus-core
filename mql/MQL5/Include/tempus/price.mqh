...
//|                                                                  |
//+------------------------------------------------------------------+
#include <tempus/WinUser32.mqh>
#include <tempus/log.mqh>
#include <tempus/compatibility.mqh>
#include <tempus/stdlib.mqh>

// a == b
bool price_equal(const double a, const double b)
{
...
// a < b
bool price_less_than(const double a, const double b)
{
    return b - a >= _Point;
}

// a > b
bool price_greater_than(const double a, const double b)
{
    return a - b >= _Point;
}
...
class aprice {
    ...
    aprice(const MqlTick &tick) { set(tick); }
};
...
// Amount of current equity to invest, 1 = all equity, 0 = no equity
double lot_size(const double lot_coef)
{
    static const double C_min_volume = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
    static const double C_max_volume = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
    static const double C_volume_step = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
    static const double C_leverage_100 = C_leverage * 100.;

    const double equity = AccountInfoDouble(ACCOUNT_EQUITY);
    double res = lot_coef * NormalizeDouble(equity / C_leverage_100, 2);
    ...
    LOG_VERBOSE("Lot size " + DoubleToString(res, DOUBLE_PRINT_DECIMALS) + ", equity " + DoubleToString(equity, DOUBLE_PRINT_DECIMALS) + ", multiplier " + 
        DoubleToString(lot_coef, DOUBLE_PRINT_DECIMALS));
    return res;
}
    ...
    const int start_out)
{
    ...
    if (ticks_len > 0 && ticks[0].time <= bar_time)
        last_tick_price.set(ticks[0]);
    else if (ticks_len > 0 && ticks[0].time > bar_time && !start_price.valid()) {
        LOG_ERROR("Using tick with time " + TimeToString(ticks[0].time, C_time_mode) + " past bar start time " + TimeToString(bar_time, C_time_mode));
        last_tick_price.set(ticks[0]);
    } else
    ...
    if (ticks_len < 1) {
        LOG_ERROR("Ticks array is empty, using opening price " + last_tick_price.to_string() + " for " + IntegerToString(duration_sec) + " bars.");
...
AveragePrice::AveragePrice(const MqlTick &ticks[], const datetime bar_time, const int duration_sec, const aprice &start_price)
{
    ...
    if (ticks_len < 1) {
        if (start_price.valid()) {
            ...
            LOG_ERROR("Ticks array is empty, using start price!");
        } else {
            value.reset();
            close_price.reset();
            LOG_ERROR("Ticks array is empty, not initialized!");
        }
        return;
    }
