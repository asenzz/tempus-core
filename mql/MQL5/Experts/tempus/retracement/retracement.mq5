...
//---
    ...
    if (predicted_price.valid()) {
        ...
        last_signal = signal;
        const double current_dev = get_indicator_value(G_iStdDev_handle, 0);
...
double OnTester()
{
    const double eq_drawdown = TesterStatistics(STAT_EQUITYDD_PERCENT);
    const double eq_drawdown_score = eq_drawdown > .01 ? 1. / eq_drawdown : 0;

    const double bn_drawdown = TesterStatistics(STAT_BALANCEDD_PERCENT);
    ...
    return sign * MathPow(MathAbs(profit), 3) * MathPow(MathAbs(eq_drawdown_score), .4) * MathPow(MathAbs(bn_drawdown_score), .4);
}
