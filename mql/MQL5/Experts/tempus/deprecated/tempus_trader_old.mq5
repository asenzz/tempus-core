...
void OnTick()
{
    ...
    const datetime current_local_time = get_windows_local_time();
    ...
    if (make_request && current_bar_time >= G_pending_request_time) {
        ...
        do req_res = controller.do_request(net, G_pending_request_time, 1, C_dataset_id_str);
        ...
        while ((!controller.get_results(net, G_pending_request_time, 1, C_dataset_id_str, results) || ArraySize(results) < 1) && get_windows_local_time() < sleep_until)
