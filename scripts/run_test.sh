...
if [[ $1 == "-d" ]]; then # Debug
  ...
  if [[ -v $MPIEXEC ]]; then
    $MPIEXEC -gdb-args "--ex 'catch throw' --ex run --directory=${PWD}/../SVRRoot --se ./${BIN}" ./${BIN} --gtest_filter="$2" 2>&1 | tee -a "${TEST_OUTPUT}"
  else
    ${DBG} --ex 'catch throw' --ex run --directory=${PWD}/../SVRRoot --se ./${BIN} --args ./${BIN} --gtest_filter="$2" 2>&1 | tee -a "${TEST_OUTPUT}"
  fi
  ...
  # $VGRIND --tool=helgrind --free-is-write=yes --history-backtrace-size=64 --max-threads=100000 --error-limit=no --log-file=./${BIN}.valgrind.log --max-stackframe=115062830400 ./${BIN} --gtest_filter="$2" 2>&1 | tee -a "${TEST_OUTPUT}"

  # Valgrind memcheck
  $VGRIND --max-threads=100000 --track-origins=yes --error-limit=no --log-file=./${BIN}.valgrind.log --leak-check=full --tool=memcheck --expensive-definedness-checks=yes --vgdb=yes --vgdb-error=1 --show-leak-kinds=definite --max-stackframe=115062830400 ./${BIN} --gtest_filter="$2" 2>&1 | tee -a "${TEST_OUTPUT}" # Enable to start GDB server on first error: --vgdb=full --vgdb-error=1
  ...
  # LD_PRELOAD=/usr/local/lib/libbytehound.so ./${BIN} --gtest_filter="$2" 2>&1 | tee -a "${TEST_OUTPUT}"
elif [[ $1 == "-p" ]]; then # Profile CPU
  PERF_DATA=${BIN}.perf.data
  SAMPLE_PERIOD=1000003
  $PERF record -b -e BR_INST_RETIRED.NEAR_TAKEN:uppp,BR_MISP_RETIRED.ALL_BRANCHES:upp -c $SAMPLE_PERIOD -o $PERF_DATA -- ./${BIN} --gtest_filter="$2" 2>&1 | tee -a "${TEST_OUTPUT}"
  # sep -start -out unpredictable.tb7 -ec BR_INST_RETIRED.NEAR_TAKEN:PRECISE=YES:SA=1000003:pdir:lbr:USR=YES,BR_MISP_RETIRED.ALL_BRANCHES:PRECISE=YES:SA=1000003:lbr:USR=YES -lbr no_filter:usr -perf-script event,ip,brstack -app ./${BIN} --gtest_filter="$2" 2>&1 | tee -a "${TEST_OUTPUT}"
  ...
  nvprof --dependency-analysis --openmp-profiling --cpu-profiling-show-library --cpu-profiling-percentage-threshold 10 --cpu-profiling --cpu-thread-tracing --trace gpu,api --replay-mode disabled --track-memory-allocations --profile-child-processes --cpu-profiling-percentage-threshold=1 -f --metrics all --replay-mode disabled --analysis-metrics --openmp-profiling --metrics achieved_occupancy --export-profile ${BIN}.nvprof ./${BIN} --gtest_filter="$1" >> "${TEST_OUTPUT}" 2>&1 & # --track-memory-allocations --metrics all --cpu-thread-tracing
  # nsys profile -r cuda,nvtx,osrt,cublas,cusolver,cusparse,openmp -o ${BIN} ./${BIN} --gtest_filter="$1" >> "${TEST_OUTPUT}" 2>&1 & # No support for Volta
  # nsys analyze ${BIN}.nsys-rep
elif [[ $1 == "-f" ]]; then # Fork
  $MPIEXEC ./${BIN} --gtest_filter="$2" >> "${TEST_OUTPUT}" 2>&1 &
