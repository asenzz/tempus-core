...
def eps_svrpath(x,y,K,Lambda,maxIterations, lambdamin, rho, RBFGamma):
    moveIdx = 0 
    start = time.time()
...
#    test_convergence_correctness(x,y,fl, Lambda, eps, Gamma=RBFGamma)
    while moveIdx < maxIterations:
        ...
        if len(ElbowLeft) == 0 and len(ElbowRight) == 0:
            ...
            eps, fl, Kstar, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, LastEventPoint, LastEvent = update_eps(eps=eps, y=y, fl=fl, K=K, Kstar=Kstar, Lambda=Lambda, 
                                                                          LeftRegion=LeftRegion, ElbowLeft=ElbowLeft, Center=Center, ElbowRight=ElbowRight, RightRegion=RightRegion, 
