...
fileNameToSave_global = "progress_state_at_apr26"
fileNameToLoad = "progress_state_at_apr26"

globaltype = np.float64
#res = { 'alphas' : [], 'gammas' : [], 'beta0' : [], 'maes' : [], 'v_maes' : [], 'v_rmse' : [], 'errors' : [], 
#       'lambdas' : [], 'ElbowLeft' : [], 'ElbowRight' : [], 'Center' : [], 'RightRegion' : [], 'LeftRegion' : [], 
#                   
#       'TimeTaken' : float('Inf'), 'fl' : [],     'LastEventPoint' : None, 'LastEvent' : None }

res = {'alphas' : [], 'gammas' : [], 'beta0' : [], 
            'maes' : [], 'v_rmse' : [], 'v_maes' : [], 
            'errors' : [], 
            'lambdas' : [], 'ElbowLeft' : [], 'ElbowRight' : [], 'Center' : [], 'RightRegion' : [], 'LeftRegion' : [], 'TimeTaken' : []}

 
def svrinit(K,y, eps, epsilon_of_epsilon):
    def dc_loss(dc_level):
        above = y - dc_level - eps;
        below = dc_level - y - eps;
        errabove = np.sum(above[above>0])
        errbelow = np.sum(below[below>0])
        return errabove + errbelow;
    # Minimize the dc loss (move a slope=0 line, we move the intercept to minimize total loss, for the initial iteration)
    minimized = scipy.optimize.minimize(dc_loss, np.median(y), method='BFGS')
    beta0 = minimized.x[0];    
    
    # Register the sets by the initial regression line
    # TODO: IMPORTANT: See this Eps*1e-3? This is an epsilon for the epsilon, namely the margin of which a value is considered to lie on the elbow.
    # If this is larger than Epsilon, the world will end... this is why the PATH algorithm failed so much especially for eps=1e-11, and somewhat for 1e-9.
    LeftRegion = np.argwhere(beta0 - y - eps > epsilon_of_epsilon);
    RightRegion = np.argwhere(y - beta0 - eps > epsilon_of_epsilon);
    ElbowLeft = np.argwhere(abs(y - beta0 - eps) < epsilon_of_epsilon)
    ElbowRight = np.argwhere(abs(y - beta0 + eps) < epsilon_of_epsilon)
    Center = np.argwhere(abs(y-beta0) < eps - epsilon_of_epsilon);
    
    # Compute the alphas and gammas for the center as default
    alphas = np.zeros(len(y));
    gammas = np.zeros(len(y));
    
    # Compute the alphas and gammas for the Left and Right region
    alphas[RightRegion] = 1;
    gammas[LeftRegion] = 1;
    
    ler = len(ElbowRight);
    lel = len(ElbowLeft);
    
    # Now, compute the alphas and gammas for the Left and Right elbows. Set their slack variables.
    if ler + lel > 1:
        print('ERROR: MORE THAN ONE POINT ON LEFT AND RIGHT ELBOWS.');
        raise Exception('ERROR: MORE THAN ONE POINT ON LEFT AND RIGHT ELBOWS.');
    # Now choose which point to pick to go on an elbow:
    # If we have more points on the left, move any right elbow pts to just outside the right elbow.
    # If we have more points on the right, move any left elbow points to just outside the left elbow.
    if np.sum(alphas[RightRegion]) > np.sum(gammas[LeftRegion]):
        gammas[ElbowLeft] = 1;
    elif np.sum(alphas[RightRegion]) < np.sum(gammas[LeftRegion]):
        alphas[ElbowRight] = 1;
    
    
    return LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, dc_loss(beta0);
...
def svrload_lambda():
    ...
    lambdas =  resultsMatrixToLoad['lambdas'][numRows]
...
#    LastEvent = resultsMatrixToLoad['LastEvent']
    ...
    return lambdas, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, mae
#, LastEventPoint, LastEvent;


def svrpath(x,y,xValidation, yValidation, K,eps, RBF_Gamma, maxIterations, lambdamin, rho, loadPreviousComputed):
...
#    if (loadPreviousComputed == True):
#        Lambda, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, betal0, error = svrload_lambda();
...
#    bm = SolveKstar(Kstar)[1:];
    ...
    while moveIdx < maxIterations and Lambda > lambdamin:
        moveIdx += 1
        if(moveIdx >= 2456):
                print('\n\n\The problematic index: ',moveIdx,' Here: \n\nElbowLeft\n',ElbowLeft)
                print('ElbowRight',ElbowRight) 
        print('\n\n\t================= Iteration-Lambda',moveIdx,'===================')
#        print('Master Iteration:',moveIdx)
...
#            print('Nonempty elbows, do standard algo')
            print('Nonempty elbows: ' + str(len(ElbowLeft)) + ' : ' + str(len(ElbowRight)))
...
#            print('b',b)
            ...
            vecR = -alphas[ElbowRight] / bstarselected[rightElbowIdces]
            vecL = gammas[ElbowLeft] / bstarselected[leftElbowIdces]   # TODO: /mnt/hdd/work-projects/svrpath-python/svrpath.py:379: RuntimeWarning: divide by zero encountered in true_divide
            lambdasRight = np.concatenate((vecR,vecL)) + Lambda
            lambdasRight[np.abs(bstarselected) < rho] = 1
            lambdasLeft = np.asarray(list(1/bstarselected[rightElbowIdces]) + list(-1/bstarselected[leftElbowIdces])) + lambdasRight
            lambdasLeft[np.abs(bstarselected) < rho] = 1
...
#            print('lambdaexit',lambdaExiting)
            ...
            leftElbowIdces = np.nonzero(np.in1d(elbows_ordered,ElbowLeft))
            if len(ElbowRight) > 0:
                alphas[ElbowRight] -= (Lambda - lambdaNew) * bstarselected[rightElbowIdces]
            if len(ElbowLeft) > 0:
                gammas[ElbowLeft] += (Lambda - lambdaNew) * bstarselected[leftElbowIdces]
...
#            print('movefrom',movefrom,type(movefrom))
        ...
        mae, errors = calcStats(y, fl, eps)
        if(moveIdx % 5 == 0):
            v_rmse, v_mae = comb_epspath.validate(validationVectors = xValidation, 
                        validationGroundTruth = yValidation, trainingVectors = x, beta0 = beta0, 
                        alphas = alphas, gammas = gammas, Lambda = Lambda, rbf_gamma = RBF_Gamma)
            AllBetaZeros.append(beta0)
            maes.append(mae)
            AllvRMSE.append(v_rmse)
            AllvMAES.append(v_mae)          
...
#
#            res['alphas'].append(AllAlphas)
#            res['gammas'].append(AllGammas)
#            res['beta0'].append(AllBetaZeros)
#            res['ElbowLeft'].append(elbowL)
#            res['ElbowRight'].append(elbowR)
#            res['Center'].append(centers)
#            res['LeftRegion'].append(lefts)  
#            res['RightRegion'].append(rights)                
#            res['maes'].append(mae)
#            res['v_rmse'].append(v_rmse)
#            res['v_maes'].append(v_mae)        
#            res['errors'].append(errors)
#            res['lambdas'].append(lambdas)
#            saveprogressstate(res, fileNameToSave_global, start, lengthTillLast=0);
                             
        print('\nEps-Lambda-MAES value after running Lambda-2D-path iteration.',' Eps:', eps, ' Lambda:', Lambda, 'MAES:', mae )           
#        AllAlphas.append(alphas)
#        AllGammas.append(gammas)
#        AllBetaZeros.append(beta0)
#        maes.append(mae)
...
#    print('Algo done.')
    ...
    h, m = divmod(m, 60)
    timeTaken = ("%d:%02d:%02d" % (h, m, s))
    return { 'alphas' : np.asarray(AllAlphas), 'gammas' : np.asarray(AllGammas), 'beta0' : np.asarray(AllBetaZeros), 
            'maes' : np.asarray(maes), 'v_rmse' : np.asarray(AllvRMSE), 'v_maes' : np.asarray(AllvMAES), 
            'errors' : np.asarray(errTerms), 
