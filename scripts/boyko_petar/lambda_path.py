...
def empty_elbows(K, y, eps, beta0, Lambda, alphas, gammas, LeftRegion, RightRegion, Center, ElbowRight, ElbowLeft,
                 elbowPointIdx, elbowSide, eps_of_eps):
    ...
    while len(ElbowLeft) + len(
            ElbowRight) < 2:  # I don't get this condition. In the paper, they concentrate on having only unique points on the elbows, and this is not covered.
        ...
        eventsFromRight = (betaStar[cAndR] - betaStar[elbowPointIdx]) / (
        y[cAndR] - y[elbowPointIdx] - const * eps - eps)
        # (beta.star[c(Lefti, Centeri)]-beta.star[i])/(y[c(Lefti, Centeri)] - y[i] - const*eps + eps)
        cAndL = LeftRegion + Center
        eventsFromLeft = (betaStar[cAndL] - betaStar[elbowPointIdx]) / (
        y[cAndL] - y[elbowPointIdx] - const * eps + eps);
    ...
    return {'right': RightRegion, 'left': LeftRegion, 'elbowleft': ElbowLeft, 'elbowright': ElbowRight,
            'center': Center,
...
def init_event(K, y, eps, beta0, alphas, gammas, LeftRegion, RightRegion, ElbowLeft, ElbowRight, Center, eps_of_eps):
    ...
    res = empty_elbows(K, y, eps, beta0fixed, 1e50, alphas, gammas, LeftRegion, RightRegion, Center, ElbowRight,
                       ElbowLeft, elbowPointIdx, elbowSide, eps_of_eps)
...
def svr_update(x, y, K, eps, lambdamin, rho, Kstar, Lambda,LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, iteration, LastEventPoint, LastEvent):
...
#    bm = SolveKstar(Kstar)[1:];
    ...
    if len(ElbowLeft) > 0 or len(ElbowRight) > 0:
        ...
        vecL = gammas[ElbowLeft] / b[
            leftElbowIdces]  # TODO: /mnt/hdd/work-projects/svrpath-python/svrpath.py:379: RuntimeWarning: divide by zero encountered in true_divide
        ...
        lambdasForPointsExitingShoulders = np.max(
            np.concatenate(([-1], lambdasForChecking[lambdasForChecking < Lambda - rho])))
        ...
        if isImmobile and lambdasForPointsExitingShoulders < rho:
            ...
            return alphas, gammas, beta0, Lambda, ElbowLeft, ElbowRight, Center, RightRegion, LeftRegion
#            break;
        if not isImmobile:
        ...
        if lambdasForPointsEnteringShoulders.size > 0:
            ...
            if len(filteredForEntering) > 0:
                lambdaNew = np.max([lambdaNew, np.max(
                    lambdasForPointsEnteringShoulders[lambdasForPointsEnteringShoulders < Lambda - rho])])
        ...
        if lambdasForPointsExitingShoulders.size > 0:
            ...
            if len(filteredForExiting) > 0:
                ...
                lambdaExiting = np.max(
                    lambdasForPointsExitingShoulders[lambdasForPointsExitingShoulders < Lambda - rho])
            else:
        ...
        if lambdaEntering >= lambdaExiting - rho:
            ...
            if observedEvent in LeftRegion:
                ...
                for item in ElbowRight:
                    idx = np.argwhere(
                        elbows_ordered == item)  # we should get exactly one match here; no item shows up twice in ordered elbows
        ...
        ee = empty_elbows(K, y, eps, betal0 / Lambda, Lambda, alphas, gammas,
                          np.asarray(LeftRegion), np.asarray(RightRegion), np.asarray(Center), np.asarray([]),
                          np.asarray([]),
...
#alphas, gammas, beta0, Lambda, ElbowLeft, ElbowRight, Center, RightRegion, LeftRegion = 
