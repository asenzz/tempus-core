...
lengthOfTrainingWindow = 20000  #+ lengthOfValidationSet  #lengthOfValidationSet is included 
maxSlidingWindowNumber = 6
...
def trainlibmsvm_here_nusvr(gamma_rbf, Lambda, eps, svm_x, svm_y, lengthOfValidationSet, Level):
        ...
        svm_x_train = svm_x[:-lengthOfValidationSet];
        cost = 1/Lambda
        base_svm_options = '-s 4 -t 2 -n 0.5';
        gamma_opt = '-g ' + str(gamma_rbf);
        cost_opt = '-c ' + str(cost);
        svm_params = base_svm_options+' '+ gamma_opt +' '+cost_opt;
        m = svmutil.svm_train(svm_y_train, svm_x_train, svm_params)
#        print('\n :m.params.eps: ',m.params.eps)
        return m    
...
#        plt.title(fileNameModelToLoadFull + ':' + str(lengthOfTrainingWindow) + ':' + str(lengthOfSlidingWindow) + ':' + ("%.3E" % res_validation_over_window['avgVMAES']) + '\nAt hyperparameters: ' + 'Gamma-RBF:' + str(gamma_rbf) +' eps:' +  str(eps) + ' Lambda:' + str(Lambda));    
    ...
    return (sumValidationPredictionErrors / (slidingWindowNumber+1))
#    plt.figure(); 
#    plt.title(fileNameModelToLoadFull + ':' + str(lengthOfTrainingWindow) + ':' + str(lengthOfSlidingWindow) + ':' + ("%.3E" % res_validation_over_window['avgVMAES']) + '\nAt hyperparameters: ' + 'Gamma-RBF:' + str(gamma_rbf) +' eps:' +  str(eps) + ' Lambda:' + str(Lambda));    
#    plt.xlabel('Offset');
...
nu_svr_eps_selected = np.zeros((16,maxSlidingWindowNumber))
...
#    errorValidation_eps_svr = trainandvalidate_continiousmodel(gamma_rbf = gamma_rbf_chosen[iterationLevel], Lambda = Lambda_best, 
...
#    listBestErrorValidation.append(differenceError)
#    
