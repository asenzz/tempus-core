...
sys.path.insert(0, '/home/peters/Work/Tasks/FindOptimalParameters/svrpath_research/combined-path-python/')
sys.path.append(os.path.dirname('/home/peters/Work/Tasks/FindOptimalParameters/svrpath_research/combined-path-python/'))
import svmutil
...
fileNameToSave_global = "progress_state_at_apr10"
fileNameToLoad = "progress_state_at"
res = { 'alphas' : [], 'gammas' : [], 'beta0' : [], 'maes' : [], 'v_maes' : [], 'v_rmse' : [], 'errors' : [], 
       'eps' : [], 'ElbowLeft' : [], 'ElbowRight' : [], 'Center' : [], 'RightRegion' : [], 'LeftRegion' : [], 
       'TimeTaken' : float('Inf'), 'fl' : [],     'LastEventPoint' : None, 'LastEvent' : None }
#res_two = { 'alphas' : [], 'gammas' : [], 'beta0' : [], 'maes' : [], 'errors' : [], 
#       'eps' : [], 'ElbowLeft' : [], 'ElbowRight' : [], 'Center' : [], 'RightRegion' : [], 'LeftRegion' : [], 
#       'TimeTaken' : float('Inf'), 'fl' : [],     'LastEventPoint' : None, 'LastEvent' : None }
    
def saveprogressstate(res, fileNametoSave, start, lengthTillLast=0):
    end = time.time()
    ...
    if(lengthTillLast > 0):       
        ...
        for key in res:
            item = res[key]
            res_to_save.update( { key : item[len(item)-lengthTillLast : len(item)]})           
        pickle.dump(res_to_save,open(fileNametoSave +'.pickle','wb'),pickle.HIGHEST_PROTOCOL);
    else:
        pickle.dump(res,open(fileNametoSave +'.pickle','wb'),pickle.HIGHEST_PROTOCOL);
...
def trainlibmsvm(gamma_rbf, Lambda, eps, fileNameModelToLoadFull, lengthOfValidationSet):
        svm_y, svm_x = svmutil.svm_read_problem(fileNameModelToLoadFull)
...
def calculate_Kline(validationVectors, trainingVectors, RBF_Gamma):
...
def validate(validationVectors, validationGroundTruth, trainingVectors, beta0, alphas, gammas, Lambda, rbf_gamma):
    K_validation = calculate_Kline(validationVectors, trainingVectors, RBF_Gamma = rbf_gamma)
    forecasts = np.ndarray(len(validationVectors))
    for i in np.arange(0,len(forecasts)):
        forecasts[i] = beta0 + 1/Lambda * np.dot(K_validation[i,:], alphas - gammas)
...
def obtainRegionsFromLibsvm(m, Lambda):
    orig_coef = m.get_sv_coef()
    num_coef = np.asarray(orig_coef)
    mult_coef = num_coef * Lambda
    alphamask = mult_coef > 0
    alphas = alphamask * mult_coef
    alphas = abs(alphas)
    gammamask = mult_coef < 0
    gammas = gammamask * mult_coef
    gammas = abs(gammas)

    RightRegion = (alphas > (1. - rho))
    LeftRegion = (gammas > (1. - rho))
    ElbowLeft = (gammas < (1. - rho)) & (gammas > rho) 
    ElbowRight = (alphas < (1. - rho)) & (alphas > rho)
    CenterAlphas = (alphas > 0) & (alphas < rho)
    CenterGammas = (gammas > 0) & (gammas < rho)
    Center = CenterAlphas & CenterGammas
    print ("\nShape alphas-libsvm is: ", alphas.shape)
#    alphas_dense = np.zeros(shape=[len(m.x_space)])
#    gammas_dense = np.zeros(shape=[len(m.x_space)])
#    for item in RightRegion:
#        alphas_dense[item] = 
    
    return LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas


def update_eps(eps, x, y, fl, K, Kstar, Lambda, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, iteration, LastEventPoint, LastEvent):  
    if DebugPrint:
        print('\nupdate_eps')
...
#                assert(False)
    ...
    if((alphas >= 0 - rho*1e6).all()==False or (alphas <= 1 + rho*1e6).all()==False or (gammas >= 0 - rho*1e6).all()==False or (gammas <= 1 + rho*1e6).all()==False):
        print('Assert is about to get triggered.')
        return eps_new, fl, Kstar, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, LastEventPoint, LastEvent, True
#        update_eps(eps=eps, x=x, y=y, fl=fl, K=K, Kstar=Kstar, Lambda=Lambda, 
#                                                                              LeftRegion=LeftRegion, ElbowLeft=ElbowLeft, Center=Center, ElbowRight=ElbowRight, RightRegion=RightRegion, 
#                                                                              alphas=alphas, gammas=gammas, beta0=beta0, iteration=iteration, LastEventPoint=LastEventPoint, LastEvent=LastEvent, BoolStop = True)
        #test_convergence_correctness(x,y,fl, Lambda, eps, Gamma=1e-3, alphas = alphas, gammas = gammas, ElbowLeft = ElbowLeft, ElbowRight = ElbowRight)
#        res['LastEventPoint'] = LastEventPoint
#        res['LastEvent'] = LastEvent            
        #saveprogressstate(res=res, fileNametoSave = fileNameToSave_global, start = time.time(), lengthTillLast=0)
#        m = trainlibmsvm(gamma_rbf=1e-3, Lambda = Lambda, eps = eps, fileNameModelToLoadFull=fileNameModelToLoadFull, )
# trainlibmsvm(gamma_rbf, Lambda, eps, fileNameModelToLoadFull, lengthOfValidationSet):
#        LeftRegionNew, ElbowLeftNew, CenterNew, ElbowRightNew, RightRegionNew, alphasNew, gammasNew = obtainRegionsFromLibsvm(m, Lambda)
#        res_libsvm = { 'alphasNew' : [], 'gammasNew' : [], 'beta0' : [], 'maes' : [], 'errors' : [], 
#       'eps' : [], 'Lambda' : [] ,'ElbowLeftNew' : [], 'ElbowRightNew' : [], 'CenterNew' : [], 'RightRegionNew' : [], 'LeftRegionNew' : [], 
#       'fl' : [],     'LastEventPoint' : None, 'LastEvent' : None }    
#        res_libsvm['alphasNew'].append(alphasNew)
#        res_libsvm['gammasNew'].append(gammasNew)
#        #res['beta0'].append(beta0)
#        #res['maes'].append(mae)
#        #res['errors'].append(errors)
#        res_libsvm['eps'].append(eps)
#        res_libsvm['Lambda'].append(Lambda)
#        res_libsvm['ElbowLeftNew'].append(ElbowLeftNew)
#        res_libsvm['ElbowRightNew'].append(ElbowRightNew)
#        res_libsvm['CenterNew'].append(CenterNew)
#        res_libsvm['RightRegionNew'].append(RightRegionNew)
#        res_libsvm['LeftRegionNew'].append(LeftRegionNew)        
#        #res['fl'].append(fl)
#        pickle.dump(res_libsvm,open('results_new_elbows' +'.pickle','wb'),pickle.HIGHEST_PROTOCOL);
#        resultsMatrixToLoad = pickle.load(open('results_new_elbows'+'.pickle','rb'))
#        alphasAll = resultsMatrixToLoad['alphasNew']
#        numRows = len(alphasAll)
#        numRows = numRows - 1 
#        #TODO Choose which row to pick; instead of picking latest row
#        alphasNew = alphasAll[numRows]
#        eps =  resultsMatrixToLoad['eps'][numRows]
#        Lambda =  resultsMatrixToLoad['Lambda'][numRows]
#        ElbowLeftNew =  resultsMatrixToLoad['ElbowLeftNew'][numRows]
#        ElbowRightNew = resultsMatrixToLoad['ElbowRightNew'][numRows]
#        CenterNew =  resultsMatrixToLoad['CenterNew'][numRows]
#        LeftRegionNew =  resultsMatrixToLoad['LeftRegionNew'][numRows]
...
#        return eps_new, fl, Kstar, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, LastEventPoint, LastEvent
    ...
    return eps_new, fl, Kstar, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, LastEventPoint, LastEvent, False
...
def combined_svrpath(x,y,xValidation, yValidation, K,Lambda,maxIterations, lambdamin, rho, RBFGamma, loadPreviousComputed, fileNameModelToLoadFull):
    moveIdx = 0 
    start = time.time()    
    LastEventPoint = None
    LastEvent = None
    if (loadPreviousComputed == False):
        eps, Kstar, fl, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, error = svrinit(K,y,Lambda);
    else:
        eps, Kstar, fl, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, error, LastEventPoint, LastEvent= svrload(K,y,Lambda);        

#    threshold_alphas_elbows = 0.001
    ...
    while moveIdx < maxIterations:
        moveIdx += 1;
        print('\n\n================= Iteration-EPS',moveIdx,'===================')
        if len(ElbowLeft) == 0 and len(ElbowRight) == 0:
...
#            if((moveIdx < 60) or (moveIdx > 5000) ):
            eps, fl, Kstar, LeftRegion, ElbowLeft, Center, ElbowRight, RightRegion, alphas, gammas, beta0, LastEventPoint, LastEvent, BoolStop = update_eps(
                    eps=eps, x=x, y=y, fl=fl, K=K, Kstar=Kstar, Lambda=Lambda, 
                    LeftRegion=LeftRegion, ElbowLeft=ElbowLeft, Center=Center, ElbowRight=ElbowRight, RightRegion=RightRegion, 
                    alphas=alphas, gammas=gammas, beta0=beta0, iteration=moveIdx, LastEventPoint=LastEventPoint, LastEvent=LastEvent)
#            else:
#                alphas, gammas, beta0, Lambda, ElbowLeft, ElbowRight, Center, RightRegion, LeftRegion = lambda_path.svr_update(
...
#            if len(ElbowLeft) > 0 and len(ElbowRight) > 0:
#                if ((gammas[ElbowLeft] < threshold_alphas_elbows).any() or (alphas[ElbowRight] < threshold_alphas_elbows).any()  
#                and (gammas[ElbowLeft] > 1.0 - threshold_alphas_elbows).any() or (alphas[ElbowRight] > 1.0 - threshold_alphas_elbows).any()):
#                    print('\n')
#                #print('Some alpha points have lower value than the threshold on the elbow')
#                else:
#                    print('All values at the elbow points have higher alphas/gammas higher than the threshold')
#                    print('Lambda value before running Lambda-path:', Lambda)
#                    alphas, gammas, beta0, Lambda, ElbowLeft, ElbowRight, Center, RightRegion, LeftRegion = lambda_path.svr_update(
#                        x=x, y=y, K=K.copy(), eps=eps, lambdamin=lambdamin, rho=rho, Kstar=Kstar.copy(), Lambda=Lambda, 
#                        LeftRegion=LeftRegion.copy(), ElbowLeft=ElbowLeft.copy(), Center=Center.copy(), ElbowRight=ElbowRight.copy(), RightRegion=RightRegion.copy(), 
#                        alphas=alphas.copy(), gammas=gammas.copy(), beta0=beta0.copy(), iteration=moveIdx, LastEventPoint=LastEventPoint.copy(), LastEvent=LastEvent.copy())
#                    print('Lambda value after running Lambda-path:', Lambda)                    
#                    Kstar = updateKstar(K, np.zeros(shape=[1,1]), Lambda, [], [], ElbowRight, ElbowLeft)
...
#                    #print('SUM ALPHAS BEFORE/AFTER::', sum(alphas), sum(alphas_svr_update))
        ...
        if (moveIdx % 50) == 0:
            v_rmse, v_mae = validate(validationVectors = xValidation, validationGroundTruth = yValidation, trainingVectors = x, beta0 = beta0, alphas = alphas, gammas = gammas, Lambda = Lambda)        
            res['maes'].append(mae)
            res['v_rmse'].append(v_rmse)
            res['v_maes'].append(v_mae)        
            res['errors'].append(errors)
            res['eps'].append(eps)
            res['fl'].append(fl)             
        print('\nEps-Lambda-MAES value after running 2D-path iteration.',' Eps:', eps, ' Lambda:', Lambda, 'MAES:', mae )        
...
#        if(BoolStop == True):
#            print('\n\nLeaving while loop at ================= Iteration',moveIdx,'===================')
#            break        
#        if(moveIdx % 25 == 0):
#            m = trainlibmsvm(gamma_rbf=1e-3, Lambda = Lambda, eps = eps)
#           trainlibmsvm(gamma_rbf, Lambda, eps, fileNameModelToLoadFull, lengthOfValidationSet):
#            LeftRegionNew, ElbowLeftNew, CenterNew, ElbowRightNew, RightRegionNew, alphasNew, gammasNew = obtainRegionsFromLibsvm(m, Lambda)
#            print ("\nShape alphas-of: EL, EL, C: ", len(alphas), '::', len(gammas), '::', len(Center))
        if (eps < 0):
            return res            
...
#            res_replace = res['eps'][bestIteration].copy()
##            res.clear();
#            res(res_replace)
            saveprogressstate(res, fileNameToSave_global, start, lengthTillLast=0);
#            plt.figure(moveIdx)
#            plt.subplot(2,1,1)
#            plt.plot(fl,c='blue')
#            plt.plot(fl+eps,'b--',)
