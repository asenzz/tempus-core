...
def GenerateHyperParams(eps = 1e-5, basedir='/home/peters/Work/Tasks/11apr-2017-stft-8l-4days/'):
    hyperParams = {}
...
def validate(eps = 1e-5, basedir='/home/peters/Work/Tasks/11apr-2017-stft-8l-4days/'):
...
def printRMSReport(basedir='/home/peters/Work/Tasks/11apr-2017-stft-8l-4days/'):
    hyperParams = pickle.load(open(basedir+'hyperparams.pickle','rb'));
    for model in hyperParams:
...
def populateRandomSearchSlide(models, basedir, slideVariables):
    gammas = create_exp_scaled_array(2e-5, 0.2, 1000);
#    gammas = []
#    gammas.append(2.15e-5)
...
#    epsilons.append(1e-4);
    epsilons = create_exp_scaled_array(0.000098, 0.019524, 1000);
    costs = create_exp_scaled_array(1e0, 1e3, 1000);
...
#                iterationSearch -= 1; continue; # because eps is fixed
            elif typeparam == 2:
            ...
            heuristic_off = '-h 0';
            smo_eps = '-e 0.001';                                   
            svm_params = base_svm_options+' '+gamma_opt+' '+eps_opt+' '+cost_opt+' '+heuristic_off +' ' +smo_eps;
#            print(svm_params)
            
                 #choose slide
                 #add available validation data
            totalNumberSlidingIterations = slideVariables['totalNumberSlidingIterations']
            lengthOfSlidingWindow = slideVariables['lengthOfSlidingWindow']
            lengthOfTrainingWindow = slideVariables['lengthOfTrainingWindow']
            lengthOfValidationSet = slideVariables['lengthOfValidationSet']
            sumMAESValidationSlide = 0
            sumMSELibsvmValidationSlide = 0
            for slidingWindowNumber in range(0, totalNumberSlidingIterations+1):
                yToSlide = model['yToSlide']
                xToSlide = model['xToSlide']
                slidingOffset = slidingWindowNumber * lengthOfSlidingWindow              
                x = xToSlide[slidingOffset:slidingOffset+lengthOfTrainingWindow]
                y = yToSlide[slidingOffset:slidingOffset+lengthOfTrainingWindow]
                xValid = xToSlide[slidingOffset+lengthOfTrainingWindow:slidingOffset+lengthOfTrainingWindow+lengthOfValidationSet]
                yValid = yToSlide[slidingOffset+lengthOfTrainingWindow:slidingOffset+lengthOfTrainingWindow+lengthOfValidationSet]

                m = svmutil.svm_train(y, x, svm_params)
                _label, p_acc, p_val = svmutil.svm_predict(yValid, xValid, m)
                diffMAESValidation = 0
                sumMSELibsvmValidationSlide += p_acc[1]
                p_val_array_1 = np.array(p_val)
                for iterarray in range(0,15):
                    diffMAESValidation += abs(p_val_array_1[iterarray] - yValid[iterarray])
                avgMAESValidation = diffMAESValidation / 15
                sumMAESValidationSlide += avgMAESValidation
#                vec_sl = [iterationSearch,slidingWindowNumber,last_gamma,last_eps,last_cost,p_acc[1],p_acc[2], sumMAESValidationSlide / totalNumberSlidingIterations]; 
#                print(iterationSearch,model['name'],vec_sl);
#                if not math.isnan(vec_sl[3]):
#                    results[model['name']].append(vec_sl);                     
#            vec = [iterationSearch,last_gamma,last_eps,last_cost,p_acc[1],p_acc[2], sumMAESValidationSlide / totalNumberSlidingIterations];                
            vec = [last_gamma,last_eps,last_cost,sumMSELibsvmValidationSlide/totalNumberSlidingIterations,p_acc[2], sumMAESValidationSlide / totalNumberSlidingIterations];
#            vec = [last_gamma,last_eps,last_cost,diffMAESValidation,diffMAESValidation];

            print(iterationSearch,model['name'],vec);
            if not math.isnan(vec[3]):
                results[model['name']].append(vec);
                           
            print('\ni.' + str(iterationSearch))
            if iterationSearch % 16 == 0:
                print('Saved.')
                try:
...
def doRandomSearchSlide(basedir='/home/peters/Work/Tasks/11apr-2017-stft-8l-4days/'):
    models = []
    totalNumberSlidingIterations = 12
    initialOffset = 0
    lengthOfSlidingWindow = int(1000/2)
    lengthOfTrainingWindow = 1000
    lengthOfValidationSet = 15
    slideVariables = {'totalNumberSlidingIterations' : totalNumberSlidingIterations, 
                      'lengthOfSlidingWindow' : lengthOfSlidingWindow,
                      'lengthOfTrainingWindow' : lengthOfTrainingWindow,
                      'lengthOfValidationSet' : lengthOfValidationSet}
    
    for modelidx in range(0,levelsPlusOne):
        modelname = 'model'+str(modelidx)
        y_init, svm_x_init = svmutil.svm_read_problem(basedir+modelname+'.'+column_name+'.txt')
        #svm_x = get_numpy_x_from_svm(svm_x);                                                     
#        for slidingWindowNumber in range(0, totalNumberSlidingIterations):
        totalAvailableSlidingOffset = initialOffset+lengthOfTrainingWindow + totalNumberSlidingIterations * lengthOfSlidingWindow + lengthOfValidationSet

        y_slide = y_init[initialOffset:totalAvailableSlidingOffset+lengthOfValidationSet]
        svm_x_slide = svm_x_init[initialOffset:totalAvailableSlidingOffset+lengthOfValidationSet]
        
        model = { 'name' : modelname };
        model.update({ 'xToSlide':svm_x_slide,'yToSlide':y_slide})#,'xValid':xValid,'yValid':yValid, 'iterationSlidingWindow' : slidingWindowNumber} );
        models.append(model)
        
    return populateRandomSearchSlide(models, basedir, slideVariables);
...
def plotRMSTopology3d(percentileToObtain = 100,basedir='/home/peters/Work/Tasks/11apr-2017-stft-8l-4days/'):
...
def plotRMSTopology(basedir='/home/peters/Work/Tasks/11apr-2017-stft-8l-4days/', SystematicParamOverlay = True, EmphasizeBestRMS = True, plotSysGamma = True, percentileToObtain = 100):
    results = pickle.load(open(basedir+'populatedRandomSearch.pickle','rb'));
    i = 0;
