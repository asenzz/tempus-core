...
struct MultivalRequestRowMapper : public IRowMapper<datamodel::MultivalRequest> {
    datamodel::MultivalRequest_ptr map_row(const pqxx_tuple &row_fields) const override
    {
        ...
        p_request->resolution = row_fields["resolution"].as<bpt::time_duration>(common::C_default_resolution);
    ...
    datamodel::MultivalRequest_ptr map_row(duckdb_result& row_set, const size_t col_count, const size_t row) const override
    {
        ...
        p_request->user_name = common::dd_get_value(row_set, row, col_count, "user_name", string());
        ...
        p_request->resolution = common::dd_get_value<bpt::time_duration>(row_set, row, col_count, "resolution", common::C_default_resolution);
        p_request->value_columns = common::dd_get_value(row_set, row, col_count, "value_columns", string());
        return p_request;
    }
};
...
struct MultivalResponseRowMapper : public IRowMapper<datamodel::MultivalResponse> {
    ...
    datamodel::MultivalResponse_ptr map_row(duckdb_result& row_set, const size_t col_count, const size_t row) const override
    {
        ...
        p_response->value_column = common::dd_get_value(row_set, row, col_count, "value_column", string());
        p_response->value = common::dd_get_value(row_set, row, col_count, "value", double(0));
        return p_response;
    }
};
...
struct ValueRequestRowMapper : public IRowMapper<datamodel::ValueRequest> {
    ...
    {
        datamodel::ValueRequest_ptr p_request = ptr<datamodel::MultivalResponse>();
        ...
        p_request->value_column = common::dd_get_value(row_set, row, col_count, "value_column", string());
        return p_request;
    }
};
