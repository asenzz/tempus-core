...
public:
    ...
    datamodel::DataRow_ptr map_row(duckdb_result& row_set, const size_t col_count, const size_t row) const override
    {
        ...
        std::vector<double> values;
        for (DTYPE(num_levels) col_ix = 4; col_ix < col_count; ++col_ix)
            values.emplace_back(common::dd_get_value<double>(row_set, row, col_ix));
		...
		return ptr<datamodel::InputQueue>(
            ...
            common::dd_get_value(row_set, row, col_count, "tick_volume", double(0)),
			values
		);
	}
