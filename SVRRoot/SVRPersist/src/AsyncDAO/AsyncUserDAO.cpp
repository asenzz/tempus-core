...
namespace {
static const auto cmp_primary_key = [](datamodel::User_ptr const &lhs, datamodel::User_ptr const &rhs) -> bool {
    return reinterpret_cast<uint64_t>(lhs.get()) && reinterpret_cast<uint64_t>(rhs.get())
           && lhs->get_user_name() == rhs->get_user_name();
};
static const auto cmp_whole_value = [](datamodel::User_ptr const &lhs, datamodel::User_ptr const &rhs) -> bool {
    return reinterpret_cast<uint64_t>(lhs.get()) && reinterpret_cast<uint64_t>(rhs.get())
           && *lhs == *rhs;
};
}
...
struct AsyncUserDAO::AsyncImpl
        : AsyncImplBase<datamodel::User_ptr, DTYPE(cmp_primary_key), DTYPE(cmp_whole_value), PgUserDAO> {
    ...
    void store(datamodel::User_ptr user)
    {
...
datamodel::User_ptr AsyncUserDAO::get_by_user_name(const std::string &user_name)
{
...
std::vector<datamodel::User_ptr> AsyncUserDAO::get_all_users()
{
    const std::scoped_lock lg(pImpl.pgMutex);
    return pImpl.pgDao.get_all_users();
}

std::vector<datamodel::User_ptr> AsyncUserDAO::get_all_users_by_priority()
{
    const std::scoped_lock lg(pImpl.pgMutex);
...
int AsyncUserDAO::save(const datamodel::User_ptr &user)
{
    pImpl.cache(user);
    return 1;
}

int AsyncUserDAO::update(const datamodel::User_ptr &user)
{
    pImpl.cache(user);
    return 1;
}

int AsyncUserDAO::remove(const datamodel::User_ptr &user)
{
