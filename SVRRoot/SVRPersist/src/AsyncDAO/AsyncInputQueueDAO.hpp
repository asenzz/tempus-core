...
public:
    ...
    size_t save(const datamodel::InputQueue_ptr &queue, const boost::posix_time::ptime &start_time = boost::posix_time::min_date_time);

    size_t save_data(const datamodel::InputQueue_ptr &queue, const boost::posix_time::ptime &start_time = boost::posix_time::min_date_time);

    size_t save_metadata(const datamodel::InputQueue_ptr &queue);

    size_t update_metadata(const datamodel::InputQueue_ptr &queue);

    size_t remove(const datamodel::InputQueue_ptr &queue);

    size_t clear(const datamodel::InputQueue_ptr &queue);
