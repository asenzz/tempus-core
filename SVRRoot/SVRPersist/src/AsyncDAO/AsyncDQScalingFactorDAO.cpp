...
bool AsyncDQScalingFactorDAO::exists(const DQScalingFactor_ptr& dq_scaling_factor)
{
    std::scoped_lock lg(pImpl.pgMutex);
    return pImpl.pgDao.exists(dq_scaling_factor);
}

int AsyncDQScalingFactorDAO::save(const DQScalingFactor_ptr& dq_scaling_factor)
{
    std::scoped_lock lg(pImpl.pgMutex);
    return pImpl.pgDao.save(dq_scaling_factor);
}

int AsyncDQScalingFactorDAO::remove(const DQScalingFactor_ptr& dq_scaling_factor)
{
    std::scoped_lock lg(pImpl.pgMutex);
