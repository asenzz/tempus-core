...
public:
    ...
    int remove(const datamodel::MultivalRequest_ptr& request);
    ...
    std::deque<datamodel::MultivalResponse_ptr> get_multival_results_column(const std::string &user_name, const std::string &column_name,
            bigint dataset_id, const bpt::ptime &value_time_start, const bpt::ptime &value_time_end, const size_t resolution);

    int save(const datamodel::MultivalResponse_ptr &p_response);

    int force_finalize(const datamodel::MultivalRequest_ptr &p_request);
