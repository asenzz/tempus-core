...
PgDQScalingFactorDAO::PgDQScalingFactorDAO(common::PropertiesReader &tempus_config, dao::DataSource &data_source) :
...
datamodel::dq_scaling_factor_container_t PgDQScalingFactorDAO::find_all_by_model_id(const bigint dataset_id)
{
    ...
    std::copy(C_default_exec_policy, sfs.cbegin(), sfs.cend(), std::inserter(result, result.end()));
    return result;
}
