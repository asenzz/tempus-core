...
public:
    ...
    datamodel::Model_ptr get_by_id(bigint model_id);

    datamodel::Model_ptr get_by_ensemble_id_and_decon_level(bigint ensemble_id, size_t decon_level);

    std::deque<datamodel::Model_ptr> get_all_ensemble_models(bigint ensemble_id);

    std::deque<datamodel::OnlineSVR_ptr> get_svr_by_model_id(bigint model_id);
};
