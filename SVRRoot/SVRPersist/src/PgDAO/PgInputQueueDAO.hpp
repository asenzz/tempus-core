...
public:
    ...
    std::deque<DataRow_ptr> get_queue_data_by_table_name(const std::string &tableName,
            const bpt::ptime &timeFrom = bpt::min_date_time,
            const bpt::ptime &timeTo = bpt::max_date_time,
            size_t limit = 0);
    std::deque<DataRow_ptr> get_latest_queue_data_by_table_name(const std::string &tableName,
    ...
    size_t save(const InputQueue_ptr& p_input_queue, const bpt::ptime &start_time = bpt::min_date_time);
    size_t save_data(const InputQueue_ptr& p_input_queue, const bpt::ptime &start_time = bpt::min_date_time);
    size_t save_metadata(const InputQueue_ptr& inputQueue);
    size_t update_metadata(const InputQueue_ptr& inputQueue);
    size_t remove(const InputQueue_ptr& inputQueue);
    size_t clear(const InputQueue_ptr& inputQueue);
