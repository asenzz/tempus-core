...
public:
    explicit PgRequestDAO(common::PropertiesReader &sql_properties, dao::DataSource &data_source);
    ...
    get_multival_request(const std::string &user_name, const bigint dataset_id, const bpt::ptime &value_time_start,
                         const bpt::ptime &value_time_end) override;
    ...
    get_active_multival_requests(const std::string &user_name, const bigint dataset_id) override;
    ...
    std::deque<datamodel::MultivalResponse_ptr>
    get_multival_results_column(const std::string &user_name, const std::string &column_name, const bigint dataset_id, const bpt::ptime &value_time_start,
                                const bpt::ptime &value_time_end, const bpt::time_duration &resolution) override;
