...
THREADSAFE_DAO_CLASS_DECLARATION_HEADER (TsModelDAO, ModelDAO)
    ...
    virtual datamodel::Model_ptr get_by_id(bigint model_id);
    virtual datamodel::Model_ptr get_by_ensemble_id_and_decon_level(bigint ensemble_id, size_t decon_level);
    virtual std::deque<datamodel::Model_ptr> get_all_ensemble_models(bigint ensemble_id);
    virtual std::deque<datamodel::OnlineSVR_ptr> get_svr_by_model_id(bigint model_id);
};
