...
namespace {
const std::string new_datarow_container = "new_datarow_container";
}
...
{
    const bpt::time_period times{bpt::time_from_string("2017-05-02 22:00:00"), bpt::time_from_string("2017-05-05 05:59:00")};
    ...
    const auto test_iq_data = aci.input_queue_service.load(test_iq_name, times.begin(), times.end());
    ...
    auto etalon_decon_output_path = exec("find ../SVRRoot/SVRBusiness-tests/test/test_data -name etalon_decon_output.txt");
...
void save_datarow_containers(std::deque<datamodel::DeconQueue_ptr> &decon_queues, const std::string &path)
{
...
std::deque<svr::datamodel::DataRow::container> load_datarow_containers(const std::string &file_path)
{
    ...
    static constexpr size_t valuate_every = 100;
