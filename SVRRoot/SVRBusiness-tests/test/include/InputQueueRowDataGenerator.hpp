...
public:
    ...
    InputQueueRowDataGenerator(svr::business::InputQueueService & input_queue_service, InputQueue_ptr queue, int numberOfValueColumns, long rowsToGenerate) :
            _queue(queue),
    ...
    DataRow_ptr operator()(){

        if(isDone()){
            return DataRow_ptr(nullptr);
        }
