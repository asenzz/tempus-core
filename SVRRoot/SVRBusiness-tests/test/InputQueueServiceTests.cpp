...
TEST_F(DaoTestFixture, InputQueueWorkflow)
{
    ...
    datamodel::DataRow_ptr row1 = std::make_shared<datamodel::DataRow>(st, bpt::second_clock::local_time(), 1, 1);
    row1->set_values({0, 0, 0, 0});
    datamodel::DataRow_ptr row2 = std::make_shared<datamodel::DataRow>(fn, bpt::second_clock::local_time(), 1, 1);
    row2->set_values({1, 1, 1, 1});
...
namespace {
long constexpr testDataNumberToGenerate = 5000;
}
...
TEST_F(DaoTestFixture, InputQueueSaveQueueTest)
{
    ...
    datamodel::InputQueue_ptr queue = std::make_shared<datamodel::InputQueue>("", "simple_queue", user1->get_user_name(),
                                                                              "description", bpt::seconds(60), bpt::seconds(5), "UTC",
    ...
    /*
        PROFILE_INFO(while(!dataGenerator.isDone()) {

            aci.input_queue_service.add_row(queue, dataGenerator());

        }, "Generating " << testDataNumberToGenerate << " InputQueue rows");
    */
    //PROFILE_INFO(EXPECT_TRUE(testDataNumberToGenerate == (ssize_t) aci.input_queue_service.save(queue)), "Saving InputQueue");
    ...
    PROFILE_INFO(queueP2->set_data(aci.input_queue_service.load(queueP2->get_table_name())), "Reading InputQueue data from database");

    EXPECT_TRUE(queue->size() == queueP2->size());

    //PROFILE_INFO(EXPECT_TRUE(1 == aci.input_queue_service.remove(queue)), "Removing InputQueue table with " << testDataNumberToGenerate << " rows");
...
TEST_F(DaoTestFixture, GetColumnInFramesTest)
{
    ...
    PROFILE_INFO(oldestRecord = aci.input_queue_service.find_oldest_record(queue), "Getting oldest record");
    PROFILE_INFO(newestRecord = aci.input_queue_service.find_newest_record(queue), "Getting newest record");
...
TEST_F(DaoTestFixture, GetDBColumnsTests)
{
    ...
    datamodel::InputQueue_ptr queue = std::make_shared<datamodel::InputQueue>("", "simple_queue", user1->get_user_name(),
                                                                              "description", bpt::seconds(60), bpt::seconds(5), "UTC", std::deque<std::string>{
...
TEST_F(DaoTestFixture, TestFixInputQueueSelection)
{
    ...
    datamodel::InputQueue_ptr queue_f = std::make_shared<datamodel::InputQueue>("", "simple_queue_f", user1->get_user_name(),
                                                                                "description", bpt::seconds(60), bpt::seconds(5), "UTC", std::deque<std::string>{
                                                                                    "eenie", "meenie", "miney", "mo", "catch", "a", "tiger", "by", "its", "toe"
                                                                                });
    ...
    datamodel::InputQueue_ptr queue_t = std::make_shared<datamodel::InputQueue>("", "simple_queue_t", user1->get_user_name(),
                                                                                "description", bpt::seconds(60), bpt::seconds(5), "UTC", std::deque<std::string>{
                                                                                    "eenie", "meenie", "miney", "mo", "catch", "a", "tiger", "by", "its", "toe"
                                                                                }, true);
...
TEST_F(DaoTestFixture, InputQueueReconciliationWorkflow)
{
    ...
    for (size_t i = 0; i < 10; ++i) {
        datamodel::DataRow_ptr row1 = std::make_shared<datamodel::DataRow>(st1 + resolution * i, bpt::second_clock::local_time(), 1, 1);
        row1->set_values({0.0 + i, 0.0 + i, 0.0 + i, 0.0 + i});
        datamodel::DataRow_ptr row2 = std::make_shared<datamodel::DataRow>(st2 + resolution * i, bpt::second_clock::local_time(), 1, 1);
    ...
    ASSERT_TRUE(missing.is_initialized());
    ASSERT_EQ("2015-01-01 10:30:00"_pt, missing->first);
    ASSERT_EQ("2015-01-01 10:30:00"_pt, missing->second);


    missing = aci.input_queue_service.get_missing_hours(iq, svr::dao::TimeRange("2015-01-01 10:00:00"_pt, "2015-01-01 10:39:00"_pt));
    ASSERT_TRUE(missing.is_initialized());
    ASSERT_EQ("2015-01-01 10:10:00"_pt, missing->first);
    ASSERT_EQ("2015-01-01 10:30:00"_pt, missing->second);


    missing = aci.input_queue_service.get_missing_hours(iq, svr::dao::TimeRange("2015-01-01 09:50:00"_pt, "2015-01-01 10:39:00"_pt));
    ASSERT_TRUE(missing.is_initialized());
    ASSERT_EQ("2015-01-01 09:50:00"_pt, missing->first);
    ASSERT_EQ("2015-01-01 10:00:00"_pt, missing->second);


    missing = aci.input_queue_service.get_missing_hours(iq, svr::dao::TimeRange("2015-01-01 09:40:00"_pt, "2015-01-01 10:50:00"_pt));
    ASSERT_TRUE(missing.is_initialized());
    ASSERT_EQ("2015-01-01 10:40:00"_pt, missing->first);
    ASSERT_EQ("2015-01-01 10:51:00"_pt, missing->second);


    missing = aci.input_queue_service.get_missing_hours(iq, svr::dao::TimeRange("2015-01-01 09:40:00"_pt, "2015-01-01 10:50:00"_pt));
    ASSERT_TRUE(missing.is_initialized());
    ASSERT_EQ("2015-01-01 09:40:00"_pt, missing->first);
    ASSERT_EQ("2015-01-01 09:50:00"_pt, missing->second);
