...
{
    auto genDesc = boost::program_options::options_description("Web service options");
    genDesc.add_options()
    ("help",
...
int main(int argc, char **argv)
{
    try {
        ...
        svc.applications_pool().mount(applications_factory<svr::web::ConnectorController>(), cppcms::mount_point("/tempus(/(.*))?"));
        svc.applications_pool().mount(applications_factory<svr::web::MainController>()), cppcms::mount_point("/web(/(.*))?");
        LOG4_DEBUG(
            "Web application has started at " << svc.settings().at("service.api").str() << "://" << svc.settings().at("service.ip").str() << ":" << svc.settings().at("service.port").number(
            ));
        svc.run();
