...
{
    return form::validate()  && validate_lookback_time();
}
...
std::vector<std::pair<std::string, std::string> > DatasetForm::get_wavelet_filters()
{
    ...
    waveletFilters.emplace_back(std::make_pair("Haar", "haar"));

    // Daubechies : db1,db2,.., ,db15
    for (int i = 1; i <= 15; i++)
        waveletFilters.emplace_back(std::make_pair("(Daubechies) db" + std::to_string(i), "db" + std::to_string(i)));
    ...
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior1.1", "bior1.1"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior1.3", "bior1.3"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior1.5", "bior1.5"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior2.2", "bior2.2"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior2.4", "bior2.4"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior2.6", "bior2.6"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior2.8", "bior2.8"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior3.1", "bior3.1"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior3.3", "bior3.3"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior3.5", "bior3.5"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior3.7", "bior3.7"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior3.9", "bior3.9"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior4.4", "bior4.4"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior5.5", "bior5.5"));
    waveletFilters.emplace_back(std::make_pair("(Biorthogonal) bior6.8", "bior6.8"));

    // Coiflets: coif1,coif2,coif3,coif4,coif5
    for (int i = 1; i <= 5; i++)
        waveletFilters.emplace_back(std::make_pair("(Coiflets) coif" + std::to_string(i), "coif" + std::to_string(i)));

    // Symmlets: sym1,sym2,........, sym20
    for (int i = 1; i <= 20; i++)
        waveletFilters.emplace_back(std::make_pair("(Symmlets) sym1" + std::to_string(i), "sym1" + std::to_string(i)));
...
void DatasetWithForm::load_form_data()
{
    ...
    object->set_steps(form.chunk_size.value());
}

bool DatasetForm::validate_lookback_time()
{
    try {
        const auto lookbackTime = bpt::duration_from_string(lookback_time.value());
        if (lookbackTime < bpt::seconds(0)) THROW_EX_FS(std::logic_error, "Lookback time must have positive value!");
