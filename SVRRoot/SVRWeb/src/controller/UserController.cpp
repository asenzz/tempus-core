...
void UserController::show(std::string user_name)
{
    ...
    const auto user_ = APP.user_service.get_user_by_user_name(user_name);
...
{
    return_result(APP.user_service.get_all_users());
}
...
void UserController::handle_create_post()
{
    ...
    if (user.form.validate()) {
        user.load_form_data();
        if (APP.user_service.exists(user.object->get_user_name())) {
            user.form.username.error_message("The username is already taken!");
            user.form.username.valid(false);
        } else if (APP.user_service.save(user.object) == 0) {
