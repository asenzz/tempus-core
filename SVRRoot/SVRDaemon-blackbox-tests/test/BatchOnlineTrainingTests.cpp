...
TEST_F(DaoTestFixture, backtest_xauusd)
{
    ...
    for (auto new_iq_iter = new_iq->begin(); new_iq_iter != new_iq->end(); ++new_iq_iter) {
        ...
        di.trigger_next_iteration();
        di.wait_iteration_finished();
        di.trigger_next_iteration();
        di.wait_iteration_finished();
        ...
        di.wait_iteration_finished();
        di.trigger_next_iteration();
        PROFILE_INFO(di.wait_iteration_finished(), "Predicting " << pred_time);
        ...
        if (mae != common::C_bad_validation and mae_ohlc != common::C_bad_validation) {
            ...
            LOG4_INFO(
                "Cycle " << ctr << " trained until " << iq->get_data().back()->get_value_time() << ", predicted " << pred_time << ", hit rate " << hit_rate_pct <<
                " pct, last known hit rate " << last_hit_pct << " pct, MAE " << cur_mae << ", last known MAE " << cur_last_mae << ", alpha " << 100. * (cur_last_mae/cur_mae - 1.) <<
                " pct, MAE OHLC " << double(total_mae_ohlc) / double(ctr) << ", last known MAE OHLC " << double(total_last_known_ohlc) / double(ctr));
        } else {
    ...
    const size_t etalon_col = 0)
{
    ...
    for (auto forecast_row = forecasts.begin(); forecast_row != forecasts.end(); ++forecast_row) {
        ...
        res += abs_diff;
        LOG4_INFO(
            "Etalon col " << etalon_col << " time " << forecast_row->get()->get_value_time() << ", position " << n_items << ", error " << abs_diff << ", predicted " << forecast_val <<
            ", etalon avg " << etalon_val);
        ++n_items;
    }
    ...
    const bpt::time_duration &forecast_resolution)
{
    ...
    for (auto forecast_row = forecasts.begin(); forecast_row != forecasts.end(); ++forecast_row) {
        ...
        LOG4_INFO(
            "Forecast time " << forecast_row->get()->get_value_time() << ", position " << n_items << ", error " << abs_diff << ", predicted " << forecast_val << ", etalon avg " <<
            etalon_val);
        LOG4_INFO(
            "Forecast time " << forecast_row->get()->get_value_time() << ", position " << n_items << ", error " << last_known_diff << ", last known price " << last_known <<
            ", last known time " << last_known_time << ", etalon avg " << etalon_val);
        ++n_items;
    }
    if (n_items > 0) {
        mae /= double(n_items);
    ...
    const data_row_container &etalon)
{
    ...
    for (auto forecast_row = forecasts.begin(); forecast_row != forecasts.end(); ++forecast_row) {
        ...
        const auto forecast_val = forecast_row->get()->get_value(0);
        const auto etalon_max = *std::ranges::max_element(etalon_row->get()->get_values());
        const auto etalon_min = *std::ranges::min_element(etalon_row->get()->get_values());
        double err = 0.;
        if (forecast_val < etalon_min) err = etalon_min - forecast_val;
        else if (forecast_val > etalon_max) err = forecast_val - etalon_max;
        mae += err;
        LOG4_INFO(
            "Time " << forecast_row->get()->get_value_time() << ", position " << n_items << ", error " << err << ", predicted " << forecast_val << ", etalon high " << etalon_max <<
            ", etalon low " << etalon_min);
        ++n_items;
    }
    if (n_items > 0) mae /= double(n_items);
    else mae = common::C_bad_validation;
    ...
    const bpt::time_duration &forecast_resolution)
{
    ...
    for (auto forecast_row = forecasts.begin(); forecast_row != forecasts.end(); ++forecast_row) {
        ...
        const auto etalon_max = *std::ranges::max_element(etalon_aux_vals);
        ...
        LOG4_INFO(
            ...
            ", etalon low " << etalon_min);
        LOG4_INFO(
            "Time " << forecast_row->get()->get_value_time() << ", position " << n_items << ", last known error " << last_known_err << ", predicted " << last_known << ", last known time " <<
            last_known_time << ", etalon high " << etalon_max << ", etalon low " << etalon_min);
        ++n_items;
    }
    if (n_items > 0) {
        mae /= double(n_items);
    ...
    const datamodel::InputQueue_ptr &new_iq_ohlc_aux)
{
    ...
    auto [mae, last_known_mae] = compare_by_value_mean_erroraux(forecasts, new_iq_ohlc_aux->get_data(), new_iq->get_resolution());
    auto [mae_ohlc, last_known_ohlc] = compare_by_value_error_ohlcaux(forecasts, new_iq_ohlc_aux->get_data(), new_iq->get_resolution());
