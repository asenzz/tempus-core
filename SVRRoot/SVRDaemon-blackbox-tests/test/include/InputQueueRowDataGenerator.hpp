...
class InputQueueRowDataGenerator{
    ...
    datamodel::InputQueue_ptr _queue;
...
public:
    ...
    InputQueueRowDataGenerator(svr::business::InputQueueService & input_queue_service, datamodel::InputQueue_ptr queue, int numberOfValueColumns, long rowsToGenerate) :
            _queue(queue),
    ...
    datamodel::DataRow_ptr operator()(){

        if(isDone()){
            return datamodel::DataRow_ptr(nullptr);
        }
