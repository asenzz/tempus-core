...
{
    if (X->values.empty()) throw std::runtime_error("Initialization vector is empty!");
    else values = X->values;
    update_values_ptr();
}

template<class T>
vektor<T>::vektor(const std::vector<T> &X)
        : vektor(X.size())
{
    resize(X.size());
    arma::copy(X.begin(), X.end(), values.begin());
}

template<class T>
vektor<T>::vektor(const arma::vec<T> &arma_vec, const ssize_t new_length)
        : values(0, viennacl::context(viennacl::MAIN_MEMORY)),
          length(new_length), max_length(vcl.size())
{/*
...
template<class T>
vektor<T>::vektor(const T value, const ssize_t new_length)
...
template<class T>
const arma::vec<T> &vektor<T>::get_values() const
{
    if (length != max_length) throw std::runtime_error(
...
void vektor<T>::copy_to(std::vector<T> &stl_vec)
{
    ...
    viennacl::vector_base<T> vcl(values.memptr(), size(), 0, 1);
...
template<typename T>
void vektor<T>::copy_to(arma::vec<T> &v) const
{
...
T* &vektor<T>::handle()
{
    return values.memptr();
}

template<class T>
const double*  &vektor<T>::handle() const
{
    return values.memptr();
}
...
{
    values_ptr = reinterpret_cast<T *>(this->values.memptr());
}
...
template<class T>
void vektor<T>::set_value(const ssize_t sample_index, T const &value)
{
#ifndef NDEBUG
    if (sample_index < 0 || sample_index >= length)
        throw std::range_error(svr::common::formatter() << "Vector index " << sample_index << " out of bounds.");
#endif
    values_ptr[sample_index] = value;
}
...
{
    /*cilk::reducer<cilk::op_add<size_t>>*/ size_t red_count = 0;
    /*cilk_*/for (decltype(length) i = 0; i < length; ++i)
        if (svr::common::equal_to(values_ptr[i], value, epsilon))
            red_count += 1;
    return red_count;
}
...
vektor<T>::remove_at(const ssize_t index)
{
    if (length > 0 && index >= 0 && index < length) {
        ...
        throw std::range_error(svr::common::formatter() << "It's impossible to remove element " << index << " from the vector that doesn't exist.");
}
...
{
    if (length < 1 || start_index < 0 || start_index >= length || end_index < 0 || end_index >= length || start_index > end_index)
...
vektor<T>::extract(const ssize_t from_index, const ssize_t to_index) const
{
    if (from_index >= 0 && to_index <= length - 1 && from_index <= to_index) {
        ...
        /* cilk_ */ for (ssize_t ix = from_index; ix <= to_index; ++ix)
            V->set_at(ix - from_index, values_ptr[ix]);
        return V;
    } else
        throw std::range_error(svr::common::formatter() << "Vector indexes " << from_index << " and " << to_index << " out of bounds.");
}
...
vektor<double> *vektor<T>::rand_vector(const ssize_t length_)
{
    ...
    /* cilk_ */ for (ssize_t i = 0; i < length_; ++i)
        V->set_at(i, static_cast<double>(rand()) / static_cast<double>(RAND_MAX));
    return V;
}
...
//        THIS_VCL /= X;
        ...
        /* cilk_ */ for (decltype(length) i = 0; i < length; ++i)
            values_ptr[i] = svr::common::SIGN<T>(values_ptr[i]) * INF;
}
...
//    THIS_VCL = viennacl::linalg::element_pow(THIS_VCL, VCL(v2));
    /* cilk_ */ for (decltype(length) i = 0; i < length; ++i)
        values_ptr[i] = std::pow(values_ptr[i], X);
}
...
vektor<T> *vektor<T>::sum_vector(const vektor<T> *V1, const vektor<T> *V2)
{
    if (V1->length == V2->length) {
        ...
        /* cilk_ */ for (decltype(V1->length) i = 0; i < V1->length; ++i)
            V3->values_ptr[i] += V2->values_ptr[i];
        return V3;
    } else
        throw std::invalid_argument("It's impossible to sum two vectors with different length.");
}
...
//        VCL((*V3)) -= VCL((*V2));
        ...
        throw std::range_error(svr::common::formatter() <<
                "It's impossible to subtract vector two vectors with different lengths " << V1.size() << " and " << V2.size());
}

template<class T>
void vektor<T>::product_vector(vektor<T> &V)
{
    THIS_VCL = VCL(product_vector(*this, V));
}
...
template<class T>
T vektor<T>::product_vector_scalar(const vektor<T> *V)
{
...
T vektor<T>::product_vector_scalar_gpu(vektor<T> &V1, vektor<T> &V2)
{
    ...
    if (length1 == V2.length) return svr::common::dot_product(V1.values_ptr, V2.values_ptr, length1);
        ...
        throw std::invalid_argument(
                ...
                "It's impossible to multiply two vectors with different lengths " << V1.length << " and " << V2.length);
}


template<class T>
T vektor<T>::sum() const
{
    return viennacl::linalg::sum(values);
}

template<class T>
T vektor<T>::abs_sum() const
{
    return viennacl::linalg::norm_1<T>(values);
}
...
void vektor<T>::abs()
{
    const viennacl::vector<T> v = viennacl::linalg::element_fabs(values);
    viennacl::copy(v.begin(), v.end(), values.begin());
}
...
{
    T min_value = 0;
    if (length > 0) min_value = viennacl::linalg::min(values);
    return min_value;
}
...
void vektor<T>::min(T &min_value, ssize_t &min_index) const
{
    if (length > 0) {
        ...
        /* cilk_ */ for (decltype(length) i = 0; i < length; ++i)
            red_min_idx->calc_min(i, values_ptr[i]);
...
void vektor<T>::min_abs(T &min_value, ssize_t &min_index) const
{
    ...
    for (decltype(length) i = 0; i < length; ++i)
        red_min_idx->calc_min(i, svr::common::ABS<T>(values_ptr[i]));
    min_index = red_min_idx->get_index_reference();
    min_value = red_min_idx->get_reference();
}
...
{
    if (length > 0)
        return viennacl::linalg::max(values);
    else
...
void vektor<T>::max(T *max_value, int *max_index) const
{
    if (length > 0) {
        ...
        /* cilk_ */ for (decltype(length) i = 0; i < length; ++i)
            red_max_idx->calc_max(i, values_ptr[i]);
...
void vektor<T>::max_abs(T *max_value, int *max_index) const
{
    if (length > 0) {
        cilk::reducer<cilk::op_max_index<ssize_t, T>> red_max_idx;
        /* cilk_ */ for (decltype(length) i = 0; i < length; ++i)
            red_max_idx->calc_max(i, svr::common::ABS<T>(this->values_ptr[i]));
        max_index = red_max_idx->get_index_reference();
        max_value = red_max_idx->get_reference();
    } else {
        max_value = std::numeric_limits<T>::quiet_NaN();
...
vektor<T> *vektor<T>::load(const char *filename)
{
    ...
    try {
        ...
        LOG4_ERROR("Error. It's impossible to complete the load. " << ex.what());
    }
    // Close the file
    afile.close();
...
void vektor<T>::save(const char *filename) const
{
    ...
    try {
        ...
        LOG4_ERROR("Error. It's impossible to complete the save. " << ex.what());
    }
    // Close the file
    afile.close();
}
