...
#pragma omp parallel for num_threads(adj_threads(fields_str.size()))
...
#pragma omp parallel for num_threads(adj_threads(dates_str.size()))
...
#pragma omp parallel for num_threads(adj_threads(ips_str.size()))
...
TEST(path_train_predict, basic_integration)
{
    ...
    auto p_svr_parameters = std::make_shared<SVRParameters>(0, 0, "test", "test", 0, 0, 0, 1e5, 0, 0, 0, C_kernel_default_max_chunk_size, 1, e_kernel_type::PATH, 1);
    ...
    *p_labels /= labels_scaling_factor;
    ...
    PROFILE_INFO(model = std::make_shared<svr::OnlineMIMOSVR>(
                      param_set,
    ...
    PROFILE_INFO(predictions = model->predict(p_features->rows(half_train_len, p_features->n_rows - VALIDATION_LENGTH - 1), bpt::special_values::not_special),
                      "Model predict");
    ...
    PROFILE_INFO(model2 = std::make_shared<svr::OnlineMIMOSVR>(
            param_set,
    ...
    PROFILE_INFO(residual_predictions = model2->predict(p_features->rows(p_features->n_rows - VALIDATION_LENGTH - 1, p_features->n_rows - 1), bpt::special_values::not_special),
                      "Model predict residuals");
    residual_predictions *= res_scale_factor;
    PROFILE_INFO(final_predictions = model->predict(p_features->rows(p_features->n_rows - VALIDATION_LENGTH - 1, p_features->n_rows - 1), bpt::special_values::not_special),
                      "Model predict final");
