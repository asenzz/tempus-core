                  ...
                  const int n_total_samples, const int num_values_trained_batch)
{
    ...
    double mae = 0.;
    PROFILE_INFO(mae = train_predict_cycle(
    ...
    SVRParameters model_svr_parameters(
            ...
            DEFAULT_LAG);
    // PROFILE_INFO(cur_variance_diff = svr::OnlineSVR::produce_kernel_matrices_variance(model_svr_parameters, learning_data, reference_data, false),
...
//    best_parameters.set_svr_C(opt_args[1]);
    ...
    for (size_t cost_exp = 1; cost_exp < 3; ++cost_exp) {
        ...
        for (size_t kernel_param_exp = 1; kernel_param_exp < 2; ++kernel_param_exp) { // 10
            ...
            double mae = 0.;
            PROFILE_INFO(mae = train_predict_cycle_smo(
