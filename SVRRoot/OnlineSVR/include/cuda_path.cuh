...
std::pair<double, double> cu_kernel_xy(const uint32_t X_cols, const uint32_t Y_cols, const uint32_t rows, const uint32_t lag, const uint16_t dim, const uint16_t lag_TILE_WIDTH, const double lambda,
...
void distances_xy(const uint32_t X_cols, const uint32_t Xy_cols, const uint32_t rows, const uint32_t lag, const double lambda, const double tau, const double min_Z, const double max_Z, CRPTRd X, CRPTRd Xy, RPTR(double) Z);

std::pair<double, double> kernel_xx(const uint32_t cols, const uint32_t rows, const uint32_t lag, const double gamma, const double lambda, const double tau, const double *const X, double *Z);

void kernel_xy(
    const uint32_t X_cols, const uint32_t Xy_cols, const uint32_t rows, const uint32_t lag, const double tau, const double lambda, const double gamma, const double min_Z, const double max_Z,
