...
public:
    ...
    explicit oemd_coefficients_search(uint16_t levels, float residual_coef, const bpt::time_duration &resolution, uint32_t label_len);

    double evaluate_mask(double att, double fp, double fs, uint32_t qt, const std::span<double> &workspace, uint8_t siftings, uint32_t prev_masks_len,
                         uint16_t mask_ix, double meanabs_input, const std::vector<uint32_t> &times, const std::vector<t_label_ix> &label_ixs, const std::vector<t_feat_params> &feat_params) const;
