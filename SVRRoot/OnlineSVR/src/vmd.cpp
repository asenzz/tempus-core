     ...
     const int K, const int DC, const int init, const double tol, const double eps, const svr::business::t_iqscaler &scaler)
{
    ...
    */
    ...
    for (size_t i = 0; i < T; ++i) {
        if (i < T / 2) {
            f[i] = scaler((iterin + T_2 - 1 - i)->get()->at(input_column_index));
            f[T_3_2 + i] = scaler((iterin + T - 1 - i)->get()->at(input_column_index));
        }
        f[T_2 + i] = scaler((iterin + i)->get()->at(input_column_index));
    }
