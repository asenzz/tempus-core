...
//        PROFILE_INFO(tune_kernel_params(p_param_set, *p_xtrain, *p_ytrain), "Tune kernel params for model " << p_param_set->get_decon_level());
    }
...
#pragma omp parallel for num_threads(ixs.size()) schedule(static, 1)
    ...
    if (update_r_matrix) PROFILE_INFO(init_r_matrix(), "init_r_matrix");
}
