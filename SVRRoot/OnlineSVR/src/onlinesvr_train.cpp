...
OnlineMIMOSVR::batch_train(
        ...
        const vec_ptr &p_instance_weights,
...
#pragma omp parallel for schedule(static, 1) num_threads(adj_threads(num_chunks))
        for (unsigned i = 0; i < num_chunks; ++i) {
            ...
            if (p_kernel_matrices->at(i).empty()) p_kernel_matrices->at(i) = *prepare_K(ccache(), *p_params, train_feature_chunks_t[i], last_value_time);
            else LOG4_DEBUG("Using pre-calculated kernel matrix for " << i);

            calc_weights(i, PROPS.get_stabilize_iterations_count(), C_solve_opt_iters);
        }
    }
    update_all_weights();
...
#pragma omp parallel for num_threads(adj_threads(p_features->n_rows * new_x.n_rows)) collapse(2)
    ...
    tbb::concurrent_map<unsigned /* chunk */, unsigned /* affected rows */> affected_chunks;
#ifdef KEEP_PREV_WEIGHTS // TODO Review and test prev weights
...
#endif
        affected_chunks.emplace(i, found_ixs.n_rows);
    }

    // Append new distances
    LOG4_DEBUG("Affected chunks " << affected_chunks.size());
    std::atomic<unsigned> add_new_row_ix = 0;
...
//        auto &params = *get_params_ptr(chunk_ix);
        ...
        for (unsigned r = 0; r < new_ixs.n_rows; ++r)
...
#pragma omp single
        {
#pragma omp task
#if 0 // TODO Fix for weights update
            K_chunk.submat(K_chunk.n_rows - chunk_new_rows, K_chunk.n_cols - chunk_new_rows, K_chunk.n_rows - 1, K_chunk.n_cols - 1) =
                *prepare_K(params, new_chunk_features_t, new_chunk_labels);
#endif
#pragma omp task
            {
                arma::mat newold_K;
...
#pragma omp parallel for schedule(static, 1) num_threads(adj_threads(main_components.size()))
    ...
    OMP_FOR_i(affected_chunks.size()) calc_weights(affected_chunks % i, PROPS.get_online_learn_iter_limit(), PROPS.get_online_learn_iter_limit());
    update_all_weights();
    samples_trained += new_rows_ct;
    last_trained_time = last_value_time;
}

arma::mat OnlineMIMOSVR::calc_weights(const arma::mat &K, const arma::mat &labels, const double epsco, const unsigned iters_irwls, const unsigned iters_opt,
                                      const arma::mat &instance_weights)
{
    arma::mat K_epsco = K;
    if (instance_weights.n_elem) K_epsco *= instance_weights * instance_weights.t();
    K_epsco.diag().fill(epsco);
    arma::mat w;
    if (iters_irwls) {
        PROFILE_EXEC_TIME(solve_irwls(K_epsco, K, labels * C_diff_coef, w, iters_irwls), "Solve weights IRWLS for K " << arma::size(K) << ", iterations " << iters_irwls);
    }
    if (iters_opt) {
        PROFILE_EXEC_TIME(solve_opt(K, labels * C_diff_coef, w, iters_opt), "Solve weights PPrune for K " << arma::size(K) << ", iterations " << iters_opt);
    }
    return w;
}

void OnlineMIMOSVR::calc_weights(const unsigned chunk_ix, const unsigned iters_irwls, const unsigned iters_opt) // TODO Remove
{
    const auto p_params = get_params_ptr(chunk_ix);
    weight_chunks[chunk_ix] = calc_weights(p_kernel_matrices->at(chunk_ix), train_label_chunks[chunk_ix], 1. / p_params->get_svr_C(), iters_irwls, iters_opt, *p_weights);
    LOG4_TRACE("Chunk " << chunk_ix << ", level " << level << ", gradient " << gradient << ", parameters " << *p_params << ", w " << common::present(weight_chunks[chunk_ix]));
}
