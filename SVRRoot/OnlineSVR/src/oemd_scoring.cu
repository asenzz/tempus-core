...
double
evaluate_mask(
        const size_t mask_size, const size_t expanded_size, const size_t siftings, double *h_mask, const double *dev_values,
        const cufftDoubleComplex *dev_values_fft, const size_t val_start, const size_t full_input_size,
        const cufftHandle plan_expanded_forward, const cufftHandle plan_expanded_backward, const cufftHandle plan_mask_forward, const cufftHandle plan_sift_forward, const cufftHandle plan_sift_backward,
        bool &filter, thrust::device_vector<double> &d_mask, thrust::device_vector<double> &d_expanded_mask, thrust::device_vector<double> &d_global_sift_matrix, const size_t current_level, const size_t gpu_id)
{
    double result = 0;
    cudaSetDevice(gpu_id);
...
void find_good_mask_anneal(
        ...
        cufftHandle *plan_full_forward, cufftHandle *plan_full_backward,
        cufftHandle *plan_mask_forward, cufftHandle *plan_mask_backward,
        cufftHandle *plan_sift_forward, cufftHandle *plan_sift_backward,
        std::vector<double> &good_mask, double &result, thrust::device_vector<double> &d_global_sift_matrix, const size_t gpu_id)
{
...
void
gauss_smoothen_mask(
        const size_t mask_size, std::vector<double> &mask, t_drand48_data_ptr buffer, cufftHandle plan_mask_forward, cufftHandle plan_mask_backward, const size_t gpu_id)
{
        ...
        thrust::device_vector<double> &d_global_sift_matrix, const size_t current_level)
{
    svr::optimizer::loss_callback_t loss_function = [&](std::vector<double>& x) -> double
    {
        ...
        return evaluate_mask(
                ...
                plan_full_forward[l_incr], plan_full_backward[l_incr], plan_mask_forward[l_incr], plan_sift_forward[l_incr], plan_sift_backward[l_incr], filter, d_mask, d_expanded_mask,
                d_global_sift_matrix, current_level, gpu_id);
    };
