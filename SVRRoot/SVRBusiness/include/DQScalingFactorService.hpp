...
public:
    ...
    static bool match_n_set(datamodel::dq_scaling_factor_container_t &sf, const datamodel::DQScalingFactor_ptr &nf, const bool overwrite);
    ...
    void scale(const datamodel::Dataset_ptr &p_dataset, const std::deque <datamodel::DeconQueue_ptr> &aux_decon_queues, // labels and features are created from aux decon data
    ...
    static void
    scale_features(const datamodel::Dataset_ptr &p_dataset, const std::deque<datamodel::DeconQueue_ptr> &aux_decon_queues,
                   const datamodel::SVRParameters_ptr &p_head_params,
    ...
    {
        LOG4_BEGIN();
        const auto p_scaling_factor = slice(scaling_factors, levix, input_queue_column_name);
        if (p_scaling_factor.empty()) LOG4_THROW("Could not find scaling factor for " << levix << " " << input_queue_column_name);
        return labels * p_scaling_factor.begin()->get()->get_labels_factor() + p_scaling_factor.begin()->get()->get_dc_offset_labels();
    }
