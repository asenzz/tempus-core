...
{
    return get_cumulatives(*dynamic_cast<kernel::kernel_path<double> *>(kernel::IKernel<double>::get(parameters).get()), X, time);
}
    ...
    const auto prepare_f = [&params, &X, &time, &kernel_ftor, this]{
        switch (params.get_kernel_type()) {
            ...
            case datamodel::e_kernel_type::PATH:
                return kernel_ftor.distances(get_cumulatives(PCAST(kernel::kernel_path<T>, kernel_ftor), X, time));
                                 ...
                                 const bpt::ptime &X_time, const bpt::ptime &Xy_time)
{
    ...
    const auto prepare_f = [&params, &X, &Xy, &X_time, &Xy_time, &kernel_ftor, this] {
        switch (params.get_kernel_type()) {
            ...
            case datamodel::e_kernel_type::PATH:
                return kernel_ftor.distances(get_cumulatives(PCAST(kernel::kernel_path<T>, kernel_ftor), X, X_time),
                                             get_cumulatives(PCAST(kernel::kernel_path<T>, kernel_ftor), Xy, Xy_time));
