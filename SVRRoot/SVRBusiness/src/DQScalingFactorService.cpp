...
{
    std::for_each(std::execution::par_unseq, new_sf.begin(), new_sf.end(), [&](const auto &nf) { if (!match_n_set(sf, nf, overwrite)) sf.emplace(nf); });
}
        ...
        const std::string &input_queue_column_name)
{
    ...
    std::for_each(std::execution::par_unseq, scaling_factors.begin(), scaling_factors.end(),
        ...
        const std::set<size_t> &levels)
{
    ...
    datamodel::dq_scaling_factor_container_t result;
    std::for_each(std::execution::par_unseq, scaling_factors.begin(), scaling_factors.end(), [&result, &dataset_id, &levels, &input_queue_column_name](const auto &sf) {
        if ((!dataset_id || !sf->get_dataset_id() || sf->get_dataset_id() == dataset_id)
            ...
            && std::isnormal(sf->get_labels_factor())
            && std::isnormal(sf->get_features_factor())
            && common::isnormalz(sf->get_dc_offset_labels())
...
DQScalingFactorService::check(
        ...
        const datamodel::dq_scaling_factor_container_t &scaling_factors)
{
    const auto dataset_id = p_head_params->get_dataset_id();
    datamodel::dq_scaling_factor_container_t missing;
    if (slice(scaling_factors, dataset_id, p_head_params->get_input_queue_column_name(), std::set{p_head_params->get_decon_level()}).empty())
        missing.emplace(std::make_shared<datamodel::DQScalingFactor>(
                0, dataset_id, decon_queues.front()->get_input_queue_table_name(), p_head_params->get_input_queue_column_name(), p_head_params->get_decon_level()));

#pragma omp parallel for num_threads(adj_threads(decon_queues.size()))
    for (const auto &dq: decon_queues) {
        const auto found_sf = slice(scaling_factors, dataset_id, dq->get_input_queue_column_name(), feat_levels);
        ...
        if (found_sf.size() != feat_levels.size())
            for (const auto fl: feat_levels)
                if (slice(found_sf, dataset_id, dq->get_input_queue_column_name(), std::set{fl}).empty())
                    missing.emplace(std::make_shared<datamodel::DQScalingFactor>(
                            0, dataset_id, dq->get_input_queue_table_name(), dq->get_input_queue_column_name(), fl));
    }
    return missing;
...
DQScalingFactorService::calculate(
        const std::deque<datamodel::DeconQueue_ptr> &decon_queues, // Labels decon is first in queue, as order of features in row
        ...
        const datamodel::dq_scaling_factor_container_t &req_factors)
{
    ...
    // Labels
    if (slice(req_factors, labels_level, p_params->get_input_queue_column_name()).size()) {
        const auto dc_offset_labels = calc_dc_offset(labels);
        const auto labels_factor = calc_scaling_factor(labels - dc_offset_labels);
        add(res, std::make_shared<datamodel::DQScalingFactor>(
...
#pragma omp parallel for num_threads(adj_threads(dqct * feat_levels.size())) collapse(2)
    for (size_t i = 0; i < dqct; ++i) {
        for (size_t j = 0; j < feat_levels.size(); ++j) {
            ...
            if (slice(req_factors, l, p_decon_queue->get_input_queue_column_name()).empty()) continue;

            const auto dc_offset = calc_dc_offset(features.cols(i * row_len + j * lag, i * row_len + (j + 1) * lag - 1));
            const auto scaling_factor = calc_scaling_factor(features.cols(i * row_len + j * lag, i * row_len + (j + 1) * lag - 1) - dc_offset);
            add(res, std::make_shared<datamodel::DQScalingFactor>(
                        0, p_params->get_dataset_id(), p_decon_queue->get_input_queue_table_name(), p_decon_queue->get_input_queue_column_name(), labels_level,
        ...
        arma::mat &last_known)
{
    ...
    auto missing_factors = check(aux_decon_queues, p_head_params, feat_levels, p_dataset->get_dq_scaling_factors());
    if (missing_factors.size() && p_dataset->get_id()) {
        LOG4_INFO("Dataset doesn't have scaling factors, loading from database.");
        p_dataset->add_dq_scaling_factors(find_all_by_dataset_id(p_dataset->get_id()));
        missing_factors = check(aux_decon_queues, p_head_params, feat_levels, p_dataset->get_dq_scaling_factors());
    }

    scale(p_dataset, aux_decon_queues, p_head_params, missing_factors, features, labels, last_known);

    if (p_dataset->get_id())
        for_each(std::execution::par_unseq, p_dataset->get_dq_scaling_factors().begin(), p_dataset->get_dq_scaling_factors().end(),
                 [&](const datamodel::DQScalingFactor_ptr &s) { DQScalingFactorService::remove(s); DQScalingFactorService::save(s); });
}
        ...
        arma::mat &last_known)
{
    ...
    LOG4_INFO("Recalculating " << missing_factors.size() << " scaling factors for dataset " << p_dataset->get_id());
    p_dataset->add_dq_scaling_factors(calculate(aux_decon_queues, p_head_params, feat_levels, features, labels, missing_factors));

__scale:
    if (!check(aux_decon_queues, p_head_params, feat_levels, p_dataset->get_dq_scaling_factors()).empty())
        LOG4_THROW("Could not prepare scaling factors for " << *p_head_params);

    // Labels
    const auto sf_labels = slice(p_dataset->get_dq_scaling_factors(), p_dataset->get_id(), p_head_params->get_input_queue_column_name(), std::set{p_head_params->get_decon_level()});
    labels = (labels - sf_labels.begin()->get()->get_dc_offset_labels()) / sf_labels.begin()->get()->get_labels_factor();
    last_known = (last_known - sf_labels.begin()->get()->get_dc_offset_labels()) / sf_labels.begin()->get()->get_labels_factor();
    if (!common::sane(labels) || !common::sane(last_known))
        LOG4_ERROR("Scaled labels not sane, scaling factor labels " << *sf_labels.begin()->get() << ", parameters " << *p_head_params);

    if (features.empty()) return;

    scale_features(p_dataset, aux_decon_queues, p_head_params, features, p_dataset->get_dq_scaling_factors(), feat_levels);
}

void DQScalingFactorService::scale_features(
        const datamodel::Dataset_ptr &p_dataset, const std::deque<datamodel::DeconQueue_ptr> &aux_decon_queues,
        const datamodel::SVRParameters_ptr &p_head_params, arma::mat &features,
...
#pragma omp parallel for collapse(2) num_threads(adj_threads(feat_levels.size() * aux_decon_queues.size()))
    for (size_t dqix = 0; dqix < aux_decon_queues.size(); ++dqix) {
        for (size_t i = 0; i < feat_levels.size(); ++i) {
            ...
            const auto sf_features = slice(dq_scaling_factors, p_dataset->get_id(), p_head_params->get_input_queue_column_name(), std::set{l});
            if (sf_features.empty())
                LOG4_THROW("Couldn't find features for dataset id " << p_dataset->get_id() << ", column name " << p_head_params->get_input_queue_column_name() << ", level " << l);
            const auto p_sf = sf_features.begin()->get();
            ...
            if (!common::sane<double>(features.cols(col_start, col_end)))
                LOG4_ERROR("Scaled features error, scaling factor features " << *p_sf << ", index " << i << ", level " << l << ", feats " << arma::size(features) <<
                                                                             ", start col " << col_start << ", end col " << col_end);
        }
    }
}

bool DQScalingFactorService::match_n_set(datamodel::dq_scaling_factor_container_t &sf, const datamodel::DQScalingFactor_ptr &nf, const bool overwrite)
{
...
//            nf->get_input_queue_table_name() != of->get_input_queue_table_name() ||
        ...
        LOG4_DEBUG("Setting factor " << *of << " to " << *nf);
