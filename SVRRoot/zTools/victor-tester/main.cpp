...
struct Order
{
    ...
    {
        return Order({row_set["px"].as<double>(0), row_set["size"].as<double>(0), row_set["stoploss"].as<double>(0), row_set["trailingstop"].as<double>(0),
                      row_set["takeprofit"].as<double>(0)});
    }
};

bool order_px_less_px(double px, Order const& ord) { return px < ord.px; }

bool order_px_less(Order const& lhs, Order const& rhs) { return lhs.px < rhs.px; }

std::basic_ostream<char>& operator<<(std::basic_ostream<char>& ostr, Order const& order)
{
    ostr << "px: " << order.px << " size: " << order.size << " stoploss: " << order.stoploss << " trailingstop: " << order.trailingstop << " takeprofit: " << order.takeprofit;
    return ostr;
}
...
int main(int argc, char** argv)
{
    ...
    double const px_mod = o_r.px - long(o_r.px) + long(o_r.px) % 100, px_addition = o_r.px - px_mod, granularity = 100;
...
OrderRequest parse(int argc, char** argv)
{
    ...
    gen_desc.add_options()("help", "produce help message")("side,s", boost::program_options::value<std::string>()->required(),
                                                           "order side. Available sides are b and s")("px,p", boost::program_options::value<double>()->required(), "curent price")(
        "delta,d", boost::program_options::value<double>()->required(), "order price delta")("length,l", boost::program_options::value<double>()->required(), "order price length");
