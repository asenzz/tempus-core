...
void expand_the_mask(const size_t mask_size, const size_t input_size, CPTRd dev_mask, double *dev_expanded_mask)
{
...
#define CU_BLOCKS_THREADS(x_size) ((x_size) + oemd_block_size - 1) / oemd_block_size, oemd_block_size
...
//WARNING - creation of plans is not thread safe  perhaps!
    ...
    for (int i = 0; i < mask.size(); i++) {
        ...
        vec_power<<<CU_BLOCKS_THREADS(full_input_size / 2 + 1)>>>(d_zm_fft_ptr, full_input_size, siftings[i]);
        gpu_multiply_complex<<<CU_BLOCKS_THREADS(full_input_size / 2 + 1) >>>(full_input_size, d_zm_fft_ptr, d_rem_fft_ptr);
        cufft_errchk(cufftExecZ2D(plan_full_backward, d_rem_fft_ptr, d_rem_ptr));

        vec_subtract_inplace<<<CU_BLOCKS_THREADS(full_input_size)>>>(d_imf_ptr, d_rem_ptr, full_input_size);
