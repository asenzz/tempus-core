...
void Hdbscan::execute(const int minPoints, const int minClusterSize, const std::string &distanceMetric)
{
    ...
    int iNdex = 1;
    for (auto it = numClustersSet.begin(); it != numClustersSet.end(); ++it) clustersMap[*it] = iNdex++;

    vector<int> normalizedLabels(labels_.size());
#pragma omp parallel for
    for (unsigned i = 0; i < labels_.size(); ++i) normalizedLabels[i] = labels_[i] != 0 ? clustersMap[labels_[i]] : -1;
