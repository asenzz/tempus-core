...
__kernel void
path_kernel_run(__global double *X, const long input_internal_size2, const long output_internal_size2, const long nXy,
                const int dimvect, const double sigma, __global double *total_result, __global double *supermin,
                const long startX, const long startY, const long numX, const long numY, const double w_sum_sym,
                const double lambda)
{
...
//Must be 32x32 == 1024 == Nx_local, careful!
    ...
    const int mm = get_global_id(1);

    const int tx=get_local_id(0);
    const int ty=get_local_id(1);

    if ((get_group_id(0) <= get_group_id(1)) || (startX!=startY) || (numX!=numY)) {
...
*/
        ...
        if ((kk<numX) && (mm<numY)){
            ...
            if ((startX==startY) && (numX==numY)&& (get_group_id(0) < get_group_id(1))) {
                total_result[(startY + mm) * output_internal_size2 + (startX + kk)] = 1. - matrix_prod_sum / (2. * sigma * sigma);
            }
        }
    }//end if  group_id
    barrier(CLK_LOCAL_MEM_FENCE);
